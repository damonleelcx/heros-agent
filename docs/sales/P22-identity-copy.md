...
| **We run no password database and no identity provider of our own** ⚠️ **CONDITIONAL since P28** | True on a **federated** deployment, and there it is a real differentiator: no password store, no recovery flow, nothing to breach. 🔴 **False on a deployment running the `password` sign-in method**, which stores a per-account argon2id hash and offers a reset flow. Never say this sentence without knowing which method the customer will run — see [P28 identity copy](P28-identity-copy.md) §1 for the conditional wording. | absence, plus `content/identity.ts`; the console's own sign-in page swaps this assurance out on the `password` seam (`PASSWORD_ASSURANCES`), fenced by `tests/password-identity.test.mjs` |
