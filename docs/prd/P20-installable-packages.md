...
flowchart TD
  ...
  sign --> notarize["notarize (macOS) / Authenticode (Windows) — per platform-trust decision"]
...
- **OQ1 — Notarization / code-signing cost.** Apple Developer ID + notarization and a Windows (EV) Authenticode
  cert are recurring cost + an organizational identity. **Decide with the user:** pay for clean Gatekeeper/
  SmartScreen (best UX), or ship the documented one-command-clear path first and add signing later? (D3 frames
  both; the choice is the user's — cost-escalation-path forbids deciding it silently.)
