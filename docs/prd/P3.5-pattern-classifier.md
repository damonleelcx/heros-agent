...
`openspec/changes/archive/2026-07-20-p3.5-pattern-classifier/specs/pattern-classifier/`.
...
**`#` is the CANONICAL pattern number**, and it is authoritative: it is how these patterns are
referred to in conversation ("Pattern 13"), so exactly one numbering may exist. The sequence
deliberately **interleaves the groups** (Tool Use is #5, between Reflection and Planning) — it is the
canonical order, not a grouping, and the Group column is what carries group membership. The numbering
is pinned in code (`PatternInfo.Ordinal`) and asserted verbatim by
`TestCanonicalOrdinalsArePinned`, so this table and the classifier cannot drift apart.

> An earlier revision of this table numbered the rows group-by-group, which made "Pattern 5" mean
> Planning here and Tool Use everywhere else. The set was always the same 20; only the identifiers
> disagreed — which is the more dangerous of the two failures, because nothing looks wrong.
...
| 5 | Tool Use | Capability | ✅ registry-tool binding | tool-call→observe→retry trajectory |
| 6 | Planning | Control-flow | ⏳ | planning node emits a consumed task list |
| 7 | Multi-Agent Collaboration | Coordination | ✅ manager→role nodes + shared context | inter-agent dispatch at runtime |
| 8 | Memory Management | Capability | ⏳ | memory read/write against a store between turns |
| 9 | Learning & Adaptation | Governance | ⏳ | policy/weights updated from feedback |
| 10 | Goal Setting & Monitoring | Governance | ⏳ | goal-check nodes firing at runtime |
| 11 | Exception Handling & Recovery | Governance | ⏳ | error→recover trajectory |
| 12 | Human-in-the-Loop | Governance | ⏳ | human-approval pause in the trace |
| 13 | Retrieval / RAG | Capability | ✅ retriever+embed+rerank→generator | retrieval hit at runtime |
| 14 | Inter-Agent Communication | Coordination | ⏳ | validated message passing between agents |
| 15 | Resource-Aware Optimization | Governance | ✅ cost/complexity-conditioned model selection | tier-selection decisions at runtime |
| 16 | Reasoning Techniques (CoT/ToT/self-consistency/debate) | Capability | ⏳ | sample-N-then-vote; multi-path reasoning |
...
| 19 | Prioritization | Control-flow | ⏳ | ordering/scoring of tasks at runtime |
| 20 | Exploration & Discovery | Control-flow | ⏳ | branching search trajectories |

**Group sizes are unchanged** by the renumbering: control-flow 7 (#1–4, #6, #19, #20), capability 4
(#5, #8, #13, #16), coordination 2 (#7, #14), governance 7 (#9–12, #15, #17, #18). The eight with a
P3.5 structural detector are **#1, #2, #3, #5, #7, #13, #15**, plus **#4 Reflection as a capped
...
> **Status: green.** Every line below is asserted by `TestM4ExitChecklist`
> (`internal/patternclassifier/m4exit_test.go`), one sub-test per line, rather than ticked by hand.
> Run `make p35-calibration` to see the verdicts and the per-detector calibration table. The limits
> of that evidence are stated in the change's design doc under "What these tests do NOT measure".

- [x] Subgraphs receive **pattern labels with confidence**, classified **per-subgraph** (never one
      label for the whole workflow).
- [x] All **eight structural detectors** fire correctly on their signature fixtures and do not
      misfire on near-misses.
- [x] A **composite** workflow with two patterns on two subgraphs emits **both** labels, each tied
      to its own subgraph.
- [x] The **LLM fallback** runs only on ambiguous subgraphs, is **constrained to the 20-pattern
      taxonomy**, and returns a **confidence**; rule detectors take precedence.
- [x] Classification of a fully rule-covered IR is **deterministic** and makes **zero LLM calls**.
- [x] **Pattern→metric-set dispatch** works: a **RAG** subgraph selects retrieval metrics
      (relevance@k / faithfulness); a **Routing** subgraph selects **misroute-rate** — P4's
      metric-set selection keys off the label.
- [x] **Behavioral** patterns (Reflection iteration, Planning, Memory, HITL, voting) are **not
      asserted as confirmed** from structure; they are structural candidates at most, deferred to P5.
- [x] Labels are **written back to the IR** via `pattern_labels` additively (no `ir_version` MAJOR
