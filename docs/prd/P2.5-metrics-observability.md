...
"if it isn't observable, it isn't done." Collection is **auto-instrumented at the provider
gateway** so operational metrics require zero user code: every provider call that flows through
...
provider calls flow through the provider gateway — but the run is opaque.
...
- **Operational metrics would require user effort.** If collection is not auto-instrumented at the
  ...
  provider-gateway auto-instrumentation design exists to prevent.
...
and the storage decision record (three stores by shape). P2's provider gateway as the
...
### Goals
- **G1** — **Auto-instrument at the provider gateway.** Every provider call and every node
...
| **Runtime (P2)** | *Produces* the telemetry — the provider gateway is the instrumentation seam; it emits without knowing who consumes. |
...
`openspec/changes/archive/2026-07-31-p2.5-metrics-observability/specs/metrics-observability/spec.md`.

**Auto-instrumentation & collection (`metrics-observability`)**
- **FR1** — Collection SHALL be auto-instrumented at the provider gateway: every provider call
...
**Trace acquisition for hand-rolled (non-framework) agents (`metrics-observability`)**
- **FR17a** — The substrate SHALL be able to emit run/node/tool spans for an agent that uses **no
  supported graph framework** (a hand-rolled loop), by instrumenting the LLM **call sites** P1
  discovery anchors — so P4.5 has traces to attribute even when no workflow graph was discovered. This
  is what makes attribution/diagnosis framework-agnostic in practice (P4.5 FR13/FR14 depend on it;
  resolves P4.5 Q8). Two acquisition paths, and — because the richer one costs the user more than
  "do nothing" — the choice is **surfaced, not silently defaulted** (`cost-escalation-path`):
  - **(a) Auto-instrument the discovered call sites (zero user code).** Each provider-SDK call P1
    found is wrapped at its boundary and emits a node span. Node identity defaults to the call-site
    **symbol**; there are no edges and node granularity is coarse (a loop body is one node, not one
    node per iteration). Cheapest; the default when the user declares nothing.
  - **(b) A user-declared node-boundary adapter (`llm-eval.yaml`-style).** The user declares their
    in-house wrapper / loop boundaries so spans carry richer node identity (per-iteration nodes,
    sub-steps) and optionally edges. Higher fidelity, small one-time cost.
- **FR17b** — The **minimum span shape** P4.5 requires SHALL be guaranteed by either path: `node_id`
  (defaulting to the call-site symbol), `start_time` (so execution order — and therefore
  first-divergence — is derivable), `status`, and the per-node cost/latency/token attributes the
  bottleneck and rule-detector signals read. A span missing `node_id` or `start_time` SHALL be
  rejected at the emission boundary rather than silently dropped, so a hand-rolled agent's trace is
  either usable by P4.5 or loudly incomplete — never quietly half-attributed.

- **FR17c** — **Dynamic linkage signals for topology recovery.** The trace SHALL carry the runtime
  signals that recover a hand-rolled agent's node edges when no static/framework graph fully does:
  span **parent-child** nesting (the caller→callee structure at run time), a shared
  **conversation/thread/session id** attribute on the spans of one logical chain, and the **temporal +
  data** order within a run. From these, an `inferred_dynamic` edge set MAY be derived for P4.5's
  first-divergence / ablation scoping (P4.5 FR15 / Decision 8). A dynamically-observed edge SHALL be
  usable to **confirm or raise the confidence** of a statically-inferred one (P1 FR4a); a
  statically-inferred edge never observed at runtime is kept but low-confidence (reconciliation,
  P4.5 Q10). These are the cheapest, highest-signal edges (span nesting + shared thread id) and are
  the minimum-viable dynamic subset.
...
- **NFR1 — Zero-user-effort collection (the defining NFR).** 100% of operational metrics are
  collected at the provider gateway with no user code. Target: a workflow author adds **zero** lines to
...
- **Auto-instrument at the gateway (Automate the second time / observability reference domain).**
  Collection is wired once at the provider gateway seam so no workflow author ever hand-instruments a
...
- **Requires (upstream):**
  ...
  - **P2** — the provider gateway as the single instrumentation seam (structured so OTel
...
- **OQ6** — Exactly where the tag-completeness + cardinality + scrub gate lives: in the OTel
  Collector (processor pipeline) vs. an emission hook in the gateway. Leaning collector processor so
