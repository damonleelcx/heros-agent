...
read-only diagnoses P4.5 produced into **concrete Variant-Spec changes** and **proves each one
before it reaches the user**. Every diagnosis maps to a **change operator** that emits candidate
Variant Specs (upgrade/downgrade model, rewrite prompt grounded in failing cases, switch context
policy, tune RAG, add/fix a tool, prune a redundant node); each candidate is ranked by **expected
gain / cost of change** under the user's hard constraints and presented as a **diff with the
diagnosis and the specific failing cases attached as evidence**. Before anything surfaces, the
**verification gate** auto-executes the proposal on the eval dataset — **held out where possible** so
...
reaches the user.** Two automation levels ship: **Advisory** (report, human applies) and **Assisted**
(one-click apply a verified proposal).
...
**Upstream state assumed:** P4 eval harness (multi-seed runs, mean + CI, significance test, the
tie-on-overlapping-CIs rule, composite score + disqualifying gates, `eval_set_hash`) — **the harness
*is* the verification engine P5.5 drives**; P4.5 attribution + diagnosis (per-node contribution,
failure clusters, typed causes with failing cases as evidence) — **the input to the operators**; P5
typed per-node I/O contracts + the Variant-Spec diff view + behavioral pattern labels + anti-pattern
diagnoses — operators emit contract-valid diffs and are **gated by the pattern label** (propose
"add rerank" only on a RAG subgraph, "add a critic" only on a chain missing Reflection); P2 Runtime
+ registries (model/prompt/skill/context) the operators reference; P3.5/P5 pattern classifier.

## 3. Goals & non-goals

### Goals
- G1. **Change-operator catalog.** Each P4.5 diagnosis maps to one or more **change operators**, each
  emitting one or more candidate Variant Specs: model-upgrade / extended-thinking, model-downgrade,
  prompt-rewrite (+ format-constraint/schema), context-policy switch (summarization / sliding window
  / reorder), RAG-tune (top-k / retriever / embedding / rerank), add-skill / fix-schema-binding, and
...
- G5. **Diff-with-evidence presentation.** Each candidate is presented as a **diff against the
  current Variant Spec**, with the originating diagnosis and the **specific failing cases** attached
  as evidence — the user sees *why*, not just *what*.
...
- G12. **Automation levels — Advisory + Assisted.** **Advisory** = report verified proposals, the
  human applies. **Assisted** = one-click apply a **verified** proposal (creating the new Variant
  Spec / promoting it). Product designs how authority is granted and shown; Assisted apply is
  available **only** for a proposal that passed the gate.
...
- **Workflow owner (end user, primary)** — has a diagnosed, under-performing workflow and wants
  **fixes they can trust**: a ranked list of concrete changes, each already proven to help, each a
  diff they can read and (Advisory) apply themselves or (Assisted) apply in one click. Never wants to
  be handed an unverified suggestion.
...
- As a workflow owner, I want each diagnosis turned into a concrete proposed change I can read as a
  diff, so that I know exactly *what* would change and *why*.
...
- As a workflow owner in **Advisory** mode, I want the verified proposal reported so I can apply it
  myself; in **Assisted** mode, I want to apply a verified proposal in **one click**.
...
- FR1. Each P4.5 diagnosis SHALL map to one or more **change operators** per the catalog: reasoning-
  ...
  SHALL emit one or more **candidate Variant Specs**.
...
- FR5. Each candidate SHALL be presented as a **diff against the current Variant Spec**, with the
  originating diagnosis and the **specific failing cases** attached as evidence.

**Verification gate (`verification`)**
- FR6. The verification gate SHALL **auto-execute** each candidate proposal against the eval dataset,
  on a **held-out split** (cases the proposal was not generated from) when such a split exists, so the
  measured gain is not overfit to the generating cases. When no held-out split exists, the gate SHALL
  surface the result as **not held-out**.
...
- FR13. **Automation levels.** The system SHALL support **Advisory** (report a verified proposal; the
  human applies) and **Assisted** (one-click apply a verified proposal, creating/promoting the new
  Variant Spec). **Assisted apply SHALL be offered only for a proposal that passed the verification
  gate**; an unverified or gate-failing proposal SHALL NOT be one-click-appliable.
...
graph LR
  ...
  OPS --> Operators
  Operators -->|candidate Variant Specs| CVAL[P5 typed-contract<br/>validation]
  ...
  subgraph Verification
    HELD[held-out auto-execution<br/>multi-seed via P4 harness]
    SIG[significance gate<br/>P4 Stats.Compare]
    REG[regression check<br/>other clusters + cost/latency budget]
  end
  VER --> Verification
  Verification -->|PASS: verdict| REC[Ranked recommendation list<br/>diagnosis+evidence+diff+verified delta]
  Verification -.->|FAIL: withheld| DROP[not surfaced]
  REC --> TREND[Trend view across variants]
  REC --> APPLY{Automation level}
  APPLY -->|Advisory| HUMAN[human applies]
  APPLY -->|Assisted| ONECLICK[one-click apply<br/>verified only]
```

**Storage (System Designer lens).**
- **Postgres** — `proposal` (`proposal_id` PK, `diagnosis_id` FK → P4.5, `operator`, base
  `variant_id`, candidate `config_hash`, `diff_blob_hash`, `status ∈ {candidate, verifying,
  verified, gate_failed, constraint_excluded}`); `proposal_evidence` (`proposal_id` FK, `case_id`,
  role ∈ {generating, held_out}); `verdict` (`proposal_id` PK, `delta`, `ci_low`, `ci_high`,
  `significant BOOL`, `held_out BOOL`, `cost_delta`, `latency_delta`, `regression_pass BOOL`,
  `cases_fixed_json`, `cases_broken_json`); `rank_entry` (proposal × ranking-context → `expected_gain`,
  `cost_of_change`, `score`, `constraint_status`). Verification runs are ordinary P4 `eval_result`
  rows tagged with the candidate `config_hash`.
- **Object store** — candidate Variant-Spec diffs, rendered candidate prompts, prompt-optimizer
  grounding bundles (failing-case traces) — all content-hashed; DB holds hashes only.
...
- `Verdict = {diff, delta±ci, significant, held_out, cost_delta, latency_delta, cases_fixed[],
  cases_broken[], regression_pass, gate_result ∈ {pass, fail_significance, fail_regression,
  fail_constraint}}`.
- `Apply(proposal, level ∈ {advisory, assisted}) → VariantSpec` (assisted requires `gate_result = pass`).
...
- *Every proposal is a Variant Spec the runtime executes.* The engine closes the loop: a
  recommendation is not an opinion about a change, it **is** a candidate Variant Spec that the P2
  runtime runs and the P4 harness scores. Recommendations are therefore **verified, not asserted** —
  the architectural backbone of the improvement engine.
...
vs. a P4-minted train/held-out tag — see Open Questions).
...
apply** is visibly gated on verification. Content is the interface: a recommendation must lead with
...
content-hashed blobs, never in logs. Assisted apply is a single reversible action (creating a new
Variant Spec, not mutating in place); the heavier P6 kill-switch/audit/rollback machinery is a P6
prerequisite, explicitly out of scope here.
...
- **Unblocks:** **P6** (the autonomous optimizer *is* this analyze → propose → verify loop made
  ...
  search, hard-constraint gates, kill switch, audit trail, and rollback).
...
| Assisted one-click apply ships an unverified change | Product / Frontend | Assisted apply enabled **only** when `gate_result = pass`; disabled-with-reason otherwise (FR13) |
...
- **Fixtures.** A diagnosed multi-pattern workflow (a Routing subgraph → per-branch Tool Use →
  ...
  accuracy, triples cost), and a **cluster-regression proposal** (fixes cluster A, breaks cluster B).
- **Operator tests.**
...
- **Automation-level tests.**
  - **Advisory:** a verified proposal is reported with its verdict; no auto-apply occurs.
  - **Assisted:** one-click apply is offered **only** for a `gate_result = pass` proposal and creates
    a new Variant Spec; a gate-failed proposal offers **no** apply control.
- **Trend-view test.** Across three variant iterations where cluster-A failures fall but cluster-B
  ...
  a false "improving."
- **UI verification.** Drive the ranked list + diff-with-evidence + trend + Advisory/Assisted screens
  against a live (stubbed-provider) verification fan-out; confirm loading/verifying/verified/gate-
  failed/error states, held-out vs. not-held-out labelling, cases-fixed/broken rendering, and that
  Assisted apply is gated on verification.
...
- [ ] Each P4.5 diagnosis maps to the catalog **change operator(s)** and emits **candidate Variant
      Specs** (upgrade/downgrade, prompt-rewrite, context-policy, RAG-tune, add-skill/fix-binding,
      prune/merge).
- [ ] Prompt rewrites are **grounded in the specific failing cases** (DSPy-style / self-refine), not
      generic; the edit is traceable to those cases.
- [ ] Operators are **gated by pattern label + the P5 typed I/O contract**: no `add rerank` on a
      non-RAG node; no contract-violating candidate emitted.
- [ ] Candidates are **ranked by expected gain / cost of change** and **respect hard constraints**
      (budget/latency/provider); a constraint-violating candidate is not surfaced as a recommendation.
- [ ] Each candidate is presented as a **diff against the current Variant Spec** with the **diagnosis
      + specific failing cases** attached as evidence.
- [ ] The gate **auto-executes** each proposal on a **held-out split** where available; the surfaced
      delta is the held-out delta (else flagged **not held-out**).
- [ ] The **significance gate** admits only statistically-real gains (multi-seed, CI, significance vs.
      baseline via the P4 primitive); a CI-overlap tie does **not** pass.
- [ ] The **regression check** catches **"fixed accuracy, tripled cost"** (cost/latency budget as a
      hard gate) **and** "fixed cluster A, broke cluster B" (other-cluster re-scoring).
- [ ] The **verdict** reports the proposed change, **measured delta with CI**, **cost/latency
      impact**, **cases fixed**, and **cases broken**.
- [ ] **Nothing unverified reaches the user** — a proposal that fails held-out / significance /
      regression / a hard constraint is **withheld**, not surfaced.
- [ ] The **ranked recommendation list** shows diagnosis + evidence + diff + **verified delta**; the
...
- [ ] **Advisory** (report, human applies) and **Assisted** (one-click apply a **verified** proposal)
      both work; Assisted apply is offered **only** for a gate-passing proposal.
...
- Q7. **Assisted apply semantics.** Does one-click apply **promote** the candidate to the workflow's
  active Variant Spec, or merely **materialize** it as a saved named variant for the user to adopt?
  (Proposed: materialize + mark as recommended; promotion to "active" is an explicit second step, so
  Assisted stays reversible and P6-Autonomous owns unattended promotion.)
