...
- [ ] Each P4.5 diagnosis maps to the catalog **change operator(s)** and emits **candidate Variant
      ...
      (ADR-001).
- [ ] The codemod is **deterministic** (same `config_hash` + same source → byte-identical diff) and
...
- [ ] The codemod is **build-preserving**: a diff that fails to compile/build is **rejected before
...
- [ ] Each candidate is presented as a **reviewable source diff** (codemod output), paired with the
      **diff against the current Variant Spec**, with the **diagnosis + specific failing cases**
      attached as evidence.
- [ ] The gate **auto-executes the transformed working copy** (the built codemod output, not a shimmed
...
- [ ] **Advisory** (open a draft PR / report the verified diff — human applies and merges) and
