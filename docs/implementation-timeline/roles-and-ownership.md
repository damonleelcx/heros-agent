...
| CLI & CI Integration (P11) | Backend + DevOps | Product (the free tier and the dry-run as a trust surface), System Designer (one substrate, one cost model), AI (local loop parity with the hosted harness), Frontend (link coverage in the console), QA (offline and egress guarantees that cannot be read from code), Sales Ops (the security-review path as part of the funnel) |
| Forge Delivery (P12) | Backend + DevOps | System Designer (credential posture, the append-only delivery record), Product (silence as the failure mode nobody reports), AI (delivery is never a path around the gate), Frontend (delivery state closes the loop), QA (the negative assertions), Sales Ops ("you grant us nothing") |
| Legal Surface & Developer Documentation (P23) | Product Designer + Frontend + Sales Operations | Backend (append-only consent records, persist-then-ack, idempotency in the schema; the CLI registry and exit-code contract the reference is generated from), System Designer (the `(kind, version, content_hash)` contract and the content-as-code ADR), DevOps (the install page as a DevOps artifact — generated from the release, verify-before-`PATH`, honest OS-trust posture, offline install; console-image delivery, air-gapped parity, live-version observability), QA (fences that ship with failing fixtures, in both directions for the CLI; the human end-to-end walk from install to first result), AI (metric definitions matching the harness; refusals documented as design, not error) |
