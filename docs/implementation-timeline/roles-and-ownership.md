...
| Legal Surface & Developer Documentation (P23) | Product Designer + Frontend + Sales Operations | Backend (append-only consent records, persist-then-ack, idempotency in the schema), System Designer (the `(kind, version, content_hash)` contract and the content-as-code ADR), DevOps (console-image delivery, air-gapped parity, live-version observability), QA (fences that ship with failing fixtures; the human end-to-end walk), AI (metric definitions matching the harness; refusals documented as design, not error) |
