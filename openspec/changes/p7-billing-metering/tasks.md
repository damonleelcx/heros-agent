...
- [x] 1.1 Add the `account` model: `customer_id`, `provider_customer_handle` (opaque; **no card
...
- [x] 1.2 Implement `ResolvePlan(plan_id) → PlanConfig` reading plan definitions (limits, SUM band,
...
- [x] 1.3 Emit a `plan_change` audit event on every plan/price-config publish; keep no price value in
      any git-tracked file (assert in CI that plan config is sourced only from the config store).
- [x] 1.4 Test: repoint a fixture plan's limit / price reference in the config store; assert the new
...
- [x] 2.1 Implement `DeriveSUM(customer, period) → Quantity` as an **aggregation of the P2.5 cost
      ...
      events (FR1).
- [x] 2.2 Add `usage_record` with **PRIMARY KEY `{customer_id, period, metric}`** for metrics
...
- [x] 2.3 Ensure SUM inherits P2/P2.5 retry-idempotency: a redelivered cost event does **not**
      double-count SUM.
- [x] 2.4 Test — **never double-count:** report a period's usage, replay the same events/records N
...
- [x] 3.1 Integrate the **Stripe-style provider**: create/manage subscriptions and report metered
...
- [x] 3.2 Implement `Charge(customer, period, kind, idempotency_key)` for `subscription` and
...
- [x] 3.3 Source provider API keys + webhook signing secrets from a **secrets manager**; never place
...
- [x] 3.4 Assert **no-resale:** runs use the **customer's own provider keys**; no invoice line item
      represents resold/marked-up provider tokens (FR13).
- [x] 3.5 Test: a retried metered-usage report for the same `{customer, period, metric}` yields **one**
...
- [x] 4.1 Implement `HandleWebhook(signed_payload)`: **signature-verify** before any side effect;
      **dedupe** by `provider_event_id` in `webhook_delivery` so a **redelivered webhook is processed
      once** (FR14).
- [x] 4.2 Implement `Reconcile(customer, period) → {matched, drift[]}` comparing `usage_record` to the
...
- [x] 4.3 Implement **additive** `Credit(customer, reason, ref)` / `Refund(...)`: corrections are new
      audited `billing_event` rows; **never delete or mutate** the underlying usage/invoice records
      (FR11).
- [x] 4.4 Test — **reversibility:** inject a wrong charge, correct it via credit; assert originals are
      intact, the correction is a new audited row, net is right, **no data loss** (FR11).
- [x] 4.5 Test — **reconciliation:** seed a drift; assert it is surfaced, not silently accepted (FR4,
      FR14).

## 5. Backend — Entitlement gate: plan × automation level (7a)
- [x] 5.1 Implement `CheckEntitlement(customer, feature, automation_level) → {allowed, reason?,
...
- [x] 5.2 Wire the gate into every gated surface — CLI, PR-open, dashboard, and **the P6 auto-merge
      loop** (the loop consults the gate before a merge and **falls back to open-PR** absent the
      Enterprise entitlement — FR8).
- [x] 5.3 Make every denial **explicit**: return the **named reason + upgrade plan**; **never**
      silently drop, degrade, or allow an over-limit/under-entitled action (FR7).
- [x] 5.4 Test — entitlement matrix: CLI/discovery on Free; Assisted PR denied Free / allowed Team+;
      dashboard denied Free / allowed Team+; auto-merge denied Team & Business / allowed Enterprise
      (FR5, FR8).
- [x] 5.5 Test — over-limit: an action past the plan's SUM band / seat limit is **denied with a named
      reason + upgrade path**, never silently (FR7).

## 6. AI Engineer — Verified billable savings & gainshare (7b, depends on P5.5)
- [x] 6.1 Implement `ComputeBillableSavings(customer, period) → {baseline_sum, optimized_sum, savings,
      ...
      **zero** (FR3).
- [x] 6.2 Persist `billable_savings` with `verified_delta_refs[]` + `merge_commits[]` so every billed
...
- [x] 6.3 Implement gainshare `Charge(..., kind = gainshare)` that **refuses** to raise a charge for
      savings **absent from the verified-delta ledger** (FR12) — gainshare bills a share of **VERIFIED
      savings only**.
- [x] 6.4 Test — **unverified-not-billed (load-bearing):** an **estimated / un-merged** saving raises
...
- [x] 6.5 Test — auditability: the baseline + holdout methodology behind a gainshare figure is
      reconstructable from stored refs.

## 7. Frontend + Product — Billing/usage UI, paywall, gainshare consent
- [x] 7.1 Product: design the **packaging boundary** by plan name (Free = CLI + discovery; Team+ =
...
- [x] 7.2 Product: design the **gainshare consent** flow as an informed, recorded, **revocable**
      contract — states the tradeoff plainly ("a share of savings we **verify and merge**"; shows the
      baseline + holdout method) (7b).
- [x] 7.3 Frontend: billing/usage surface — **SUM under management**, active plan + entitlements, and
      ...
      delta + merged PR).
- [x] 7.4 Frontend: first-class states — **over-limit-with-upgrade-path**, **payment-failed /
...
- [x] 7.5 Frontend: SUM + savings charts via the **dataviz** skill (contrast, light/dark); keyboard-
      reachable consent + denial states.

## 8. DevOps — Revenue observability & rollout
- [x] 8.1 Emit metering + billing signals on the **P2.5 substrate**: SUM per customer/period, metered
...
- [x] 8.2 Migrations **expand-only** (account, usage_record, billable_savings, billing_event,
      webhook_delivery); plan config ships via the **config store**, never a migration.
- [x] 8.3 Rollout: **7a** behind a billing feature flag in provider **test mode**, dark until the
...
## 9. Testing & review
- [x] 9.1 Fixtures: a synthetic customer on each named plan with **config-store** plan definitions
...
- [x] 9.2 Metering tests: SUM derived from P2.5 events + deterministic re-derivation (2.1); replayed
      period counts once (2.4); reconciliation surfaces drift (4.5).
- [x] 9.3 Entitlement tests: the plan × level matrix (5.4); over-limit denied with upgrade path (5.5);
      **plan/price change with no code deploy** (1.4).
- [x] 9.4 Billing tests: never-double-charge on retry/redelivery (3.5, 4.1); reversibility via credit
      with no data loss (4.4); no-resale (3.4).
- [x] 9.5 Gainshare tests: **unverified/estimated saving bills nothing**; verified+merged saving bills
      and traces to ledger refs + merges (6.4); baseline/holdout auditable (6.5).
- [x] 9.6 Secrets test: provider keys/webhook secrets from the secrets manager, in **no**
      span/label/log; an **unsigned** webhook is rejected before any side effect.
- [x] 9.7 UI verification: drive plan-select → usage/SUM → invoice breakdown → over-limit-with-upgrade
...
- [x] 9.8 Confirm the M10 exit checklist (PRD §13) is green — 7a first, then 7b.
