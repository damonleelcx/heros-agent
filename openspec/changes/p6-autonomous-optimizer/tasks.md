...
- [x] 1.1 Wire the **P4 composite score** (active weight profile) as the search objective and the
      **P4 gates** as hard constraints: a candidate that fails any gate is never selected/applied,
      regardless of composite score.
- [x] 1.2 Implement `Search.NextCandidates(diagnosis, current_spec, policy)` — **diagnosis-guided
...
- [x] 1.3 Record, for **every** candidate evaluated, the **motivating diagnosis** (`diagnosis_id`),
      so the diagnosis-guided sample-efficiency claim is auditable, not asserted.
- [x] 1.4 Define the blind-expansion fallback and its trigger (targeted candidates exhausted *and*
      budget remains, capped by a separate blind-search sub-budget — see design Q1).
- [x] 1.5 Test: first candidates are at the attributed node+dimension, before any blind candidate;
...
- [x] 2.1 Invoke the **P5.5 build gate + held-out verification** per iteration on the **transformed
...
- [x] 2.2 **Apply = open a PR and merge it**, **only** on a build-passing, held-out verified gain with
...
- [x] 2.3 Validate every candidate against the **P5 typed I/O contracts** before verification;
...
- [x] 3.1 Implement explicit **stopping conditions**: min-improvement threshold (marginal gain on the
      composite CI-lower-bound — design Q2), max iterations, budget ceiling.
- [x] 3.2 **Min-improvement stop:** after a candidate whose verified gain is below the threshold, stop
      iterating (state `converged`), do not continue. Test it.
- [x] 3.3 **Stall/no-progress detection:** K consecutive iterations with no gate-passing,
      verification-passing gain → stop with state `stalled`. Test it (no infinite loop).
- [x] 3.4 **Recovery:** a failed merge or crashed iteration leaves the last-good (currently-merged)
...
- [x] 4.1 **Prerequisite gate:** the merge step is **disabled** unless a kill switch, an audit trail
...
- [x] 4.2 **Kill switch:** stop the loop on demand; after it fires no candidate merges, the in-flight
...
- [x] 4.3 **Audit trail = git history + change ledger:** git records each merge commit; the append-only
...
- [x] 4.4 **Write-ahead merge:** the change-ledger event commits **before** the PR is merged (the merge
...
- [x] 4.5 **Rollback = `git revert`:** `Rollback(applied_change)` reverts the merge commit to
...
- [x] 5.1 **Constraint engine:** budget ceiling (cumulative spend cap), provider allowlist,
      min-improvement threshold, max iterations — set at grant time, **immutable** for the run.
- [x] 5.2 **Budget halt:** when cumulative run spend breaches the ceiling, halt mid-run, merge nothing
      further, **disarm** merge, record the halt. Test: drive a run past the ceiling → halts, no
      further merge.
- [x] 5.3 **Regression halt:** when regression detection finds any tracked metric degraded beyond
      threshold vs. the current best, halt and disarm merge. Test with a seeded post-merge regression.
- [x] 5.4 **Disarm-until-re-arm:** a halt disarms the merge step until a human explicitly re-arms it;
      re-arming is an explicit, audited action (design Q4).
- [x] 5.5 **Bounded fan-out / least privilege:** per-iteration verification fan-out (candidate ×
...
- [x] 6.1 **Production-failure intake:** `IngestProductionFailure(trace) → EvalCase` adds a new case
...
- [x] 6.2 Test: ingest a production-failure trace → new eval case present in the P4 set, coverage
      re-measured, next run scores against it.
- [x] 6.3 **Fail-closed degradation:** if the search controller, verification service, queue, or
...
- [x] 6.4 **Concurrency safety:** one active Autonomous run per workflow, enforced by a lock keyed on
      the workflow (no write-write hazard on the repo/live spec) — design Q6.

## 7. Product + Frontend — Autonomous-level governance UX
- [x] 7.1 Product: design the **Autonomous authority-grant** flow — the user sets the hard constraints
...
- [x] 7.2 Product: design the **unhappy paths first** — loop halted by regression, loop stopped
...
- [x] 7.3 Frontend: **live monitor** — current iteration, cumulative spend against the ceiling,
      **PRs merged**, streaming audit trail (change ledger + git history); reads P2.5 signals, never
      derived state that drifts.
- [x] 7.4 Frontend: an **always-visible stop control** wired to the kill switch — keyboard-reachable,
      never scrolled off; halts immediately.
- [x] 7.5 Frontend: a **`git revert` rollback control on every merged change**, showing the diagnosis,
      the verified delta, and the cost/latency impact; one click reverts the merge commit.
- [x] 7.6 Frontend: first-class states — running / converged / max-iter / halted-regression /
...
- [x] 7.7 Frontend: make the three automation levels (Advisory / Assisted / Autonomous) visibly
      distinct — the user always knows which contract is active and how to step down or stop.

## 8. Testing & review
- [x] 8.1 Fixtures: a multi-node workflow with two independently diagnosable defects (reasoning-heavy
      ...
      seeded regression.
- [x] 8.2 Prerequisite-gating: rollback (`git revert`) / audit (git history + ledger) / kill-switch
      absent → dry-run, **zero merges**; all armed → merge preceded by its ledger event (write-ahead).
- [x] 8.3 Diagnosis-guided search: attributed-node candidates before blind candidates; each carries
      its diagnosis. Objective/constraint: gate-failing high-scorer not merged.
- [x] 8.4 Budget-halt-mid-run; min-improvement-stop; stall-detection-stop — each stops with the
      correct state and merges nothing further.
- [x] 8.5 Rollback-via-`git revert`: merged change reverted to the byte-identical prior spec
      (`config_hash` match), revert audited in the ledger.
- [x] 8.6 Regression-halt: per-iteration build+eval+regression check blocks merge; seeded post-merge
      regression halts + disarms. Kill switch: no merge after stop, last-good live, stop audited.
- [x] 8.7 Feedback loop: production-failure trace → new P4 eval case, coverage re-measured. Fail-
      closed: change-ledger/verification store down → loop stops merging.
- [x] 8.8 UI verification: drive grant → live monitor → stop → `git revert` rollback against a live
...
- [x] 8.9 Confirm the M9 exit checklist (PRD §13) is green. Loop ships **dark, dry-run-only** (opens
