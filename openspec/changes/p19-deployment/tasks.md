...
- [ ] 1.1 Ratify **D1 (Kustomize over Helm)**, **D2 (one image set, two substrates)**, and **D5 (operator
...
- [ ] 1.2 Write the control/data-plane component inventory: which components are control plane, which are data
...
- [ ] 1.3 Confirm the **digest-pinned image set** the two substrates share (agentd, customer console, admin
...
- [ ] 2.1 Author `deploy/docker-compose.platform.yml`: agentd + Postgres + object store + queue + vector/graph
      stores + customer console, one `/readyz` truth, every secret in `${VAR:?}` refuse-to-start form.
- [ ] 2.2 Author `deploy/.env.platform.example` carrying **no secrets** (mirror the `.env.console.example`
      posture: placeholders only, "THIS FILE CARRIES NO SECRETS AND NEVER WILL").
- [ ] 2.3 Declare Postgres as a stateful component with a named volume; ship the **backup CronJob/service** and
...
- [ ] 3.1 Extend `/readyz` (`internal/api/server.go`) to aggregate **every** deployed component — the stores,
...
- [ ] 3.2 Wire the boot path (`internal/launch`) so the model-calling stages resolve credentials **only** through
...
- [ ] 3.3 Confirm the fail-closed readiness signal measures **reachability**, not traffic freshness, and does
...
- [ ] 4.1 Author `deploy/k8s/base/`: a complete, applyable description — Deployments/StatefulSets for every
...
- [ ] 4.2 Add a **PodDisruptionBudget** for every workload with replicas > 1 (a node drain cannot remove the last
      replica of a control- or data-plane service).
- [ ] 4.3 Author the **NetworkPolicy**: default-deny, allow only the seam flows (task 1.2), and make **egress an
      allowlist** — the model-call egress confined to the gateway's client; no bare unrestricted egress.
- [ ] 4.4 Reference **all images by digest**; add the mutable-tag lint that fails an overlay pointing at a
      floating tag.
- [ ] 4.5 Reference secrets via an **external-secret mechanism** (external-secrets operator / CSI / sealed
      reference); add the lint that fails on a committed plaintext `Secret`.
- [ ] 4.6 Author `overlays/{dev,staging,prod,airgapped}`, each expressing **only its differences** (replica
...
- [ ] 5.1 Author `deploy/Dockerfile.admin-console` inheriting `Dockerfile.console` posture: digest-pinned
...
- [ ] 5.2 Author `deploy/docker-compose.admin-console.yml` (ADR-006 two-container shape): admin BFF `:4310` +
...
- [ ] 5.3 Add the admin console to `deploy/k8s/base/` on its **own origin/service**; assert a **disjoint cookie
      jar** and that no admin capability is reachable from a customer-console route.
- [ ] 5.4 Aggregate the admin console's health into `/readyz` (task 3.1), naming it when degraded.

## 6. AI Engineer + DevOps — Internal LLM access posture
- [ ] 6.1 Document and wire the secret store paths: env (dev), AWS Secrets Manager (managed, per
      `secrets-baseline.md`), and an on-prem gateway target for air-gapped; provider→secret-ID mapping is
      configuration, not code.
- [ ] 6.2 Assert **egress confinement**: only the gateway's client may reach a provider; a denied off-allowlist
      call is a test.
- [ ] 6.3 Assert **no customer-path dependency**: no artifact wires the platform into a customer's production
      request path (ADR-002); the model path is internal-only.
- [ ] 6.4 Air-gapped fail-static: with the on-prem gateway unreachable, model-dependent stages report
...
- [ ] 7.1 Define the **self-contained package**: platform binaries + `docker save` images + manifests +
...
- [ ] 7.2 Make deployment **declarative-idempotent** (apply twice → same state); **upgrade = apply a new package
      with the same command**; no bespoke teardown.
- [ ] 7.3 **Rollback = re-apply the prior package**, non-destructive to upgrade-window data; **never** a
      same-version fallback.
- [ ] 7.4 Ship a **`doctor` / preflight** check and **backup + restore** procedures an operator runs without us.
...
- [ ] 8.4 Health: a deliberately-broken component (stopped store, missing credential, dead console) turns
      `/readyz` red and **names** it; probes read the endpoint, not a UI.
- [ ] 8.5 Security-as-tests: no secret in any bundle/manifest/log (build scan + apply lint); admin origin
...
- [ ] 8.6 **Idempotency**: a second apply is a no-op; a re-install says "already present" exactly once and does
...
- [ ] 9.1 Write the self-contained deploy runbook (`deploy/README.md`): each term defined before use; capacity as
...
- [ ] 9.2 Encode the **commercial boundary**: an open-core overlay standing up the self-hostable core (local
      execution layer + base control-plane modelling); managed/enterprise capabilities gated by entitlement.
- [ ] 9.3 Honesty gates: **no price value** in any manifest/doc/bundle (plans by name); **"one deployment = one
...
- [ ] 10.1 Cross-link the PRD, this change, and the ADRs it inherits (002/004/006/008); add the P19 row to
      `docs/prd/README.md`.
...
- [ ] V2 Deployment-form impact matrix attached to every P19 PR.
