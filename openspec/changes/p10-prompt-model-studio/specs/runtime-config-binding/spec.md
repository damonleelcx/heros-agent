# Runtime Config Binding — Spec Delta (P10)
...
Design reasoning: [`../../design.md`](../../design.md) Decisions 1, 2, 4, 5, 6, 7.
...
**Amended by [ADR-004](../../../../../docs/adr/ADR-004-runtime-config-binding.md).** What is forbidden is
a shim **the platform injects** around the user's call — absent from their repository, absent from
their build, and therefore not the artifact that ships. A **generated, in-repo, reviewed, built**
binding artifact is not that shim: it is part of the transformed working copy the gate executes.
Verification therefore runs with the resolver **pinned** (embedded document only, override sources
disabled), so the requirement's intent — measure the code that would actually ship — is preserved
exactly.
...
- **AND** no platform-injected shim substitutes parameters at execution time

#### Scenario: A bound candidate is verified against its embedded configuration

- **WHEN** a candidate containing `bound` nodes is verified
- **THEN** the resolver is pinned to the binding document embedded in the built artifact
- **AND** no local or remote override source is consulted
- **AND** the resolved configuration hash observed during verification equals the candidate's
  configuration hash, or the run fails.
