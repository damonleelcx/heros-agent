...
`go test ./internal/{registry,telemetry,skillgate,sandbox,broker,sandboxaudit,sandboxe2e}/`.
...
| Skill args violating the schema **rejected before running**; bad output **discarded before propagation** | `skillgate.TestCheckInput_ArgSchemaViolationNamesField`, `TestCheckOutput_ViolationFailsClosed`, `sandboxe2e.TestSandboxOutput_GatedByContractBeforePropagation` | ✅ |
...
| Needed LLM/retrieval call **brokered by the host**; isolate never holds the credential | `broker.TestComplete_HostPerformsCallIsolateHoldsNoCredential`, `sandboxe2e.TestBrokerBoundary_CredentialNeverInIsolate` | ✅ |
