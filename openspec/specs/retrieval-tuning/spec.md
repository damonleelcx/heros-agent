# Retrieval Tuning — Spec (folded from P16)
...
`OpRAGTune` variants on retrieval nodes only, verified on held-out eval sets, and measured
deterministically with the retriever pinned.
...
> from the tuning set — disjoint by construction, and an authored overlap is **refused** rather than
> de-duplicated — and every measurement run pins the retriever, its params, and the seed so the same
> hash issues the identical resolved retrieval request.

## Implementation evidence

| Requirement | Where it lives | Proof |
|---|---|---|
| Tuning proposed only on a RAG node | [`proposal/catalog.go`](../../../internal/proposal/catalog.go) `ragTuneOp.AdmissiblePatterns` | `TestAddRerank_GatedToRAGNode`, `TestRAGTuneProposesChunkAndEmbedding` |
| Top-k, chunk size, embedding model | [`proposal/catalog.go`](../../../internal/proposal/catalog.go) `ragTuneOp.Propose`, `retrievalDelta` | `TestRAGTuneProposesChunkAndEmbedding` |
| Held-out verification; overlap refused | [`proposal/retrievalverify.go`](../../../internal/proposal/retrievalverify.go) `VerifyRetrievalChange` | `TestRetrievalVerifiedOnHeldoutSet`, `TestRetrievalHeldoutFloorIsNotAPass` |
| Deterministic measurement run | [`contextassembly/measurement.go`](../../../internal/contextassembly/measurement.go) `MeasurementPin`, `SameRequest` | `TestRetrievalMeasurementDeterministic`, `TestUnpinnedRetrievalIsNotAMeasurementRun` |
| Drop gate applies to retrieval | [`proposal/dropgate.go`](../../../internal/proposal/dropgate.go) | `TestRetrievalTunePastDropToleranceInadmissible` |
| Augmentation is retrieval, not loss | [`registry/context_policies.go`](../../../internal/registry/context_policies.go) `RAGRetrievalPolicy` | `TestAugmentationIsNotDrop` |
| Reduction legible through the standard family | consumed from [`evalharness/metricnames.go`](../../../internal/evalharness/metricnames.go) | `TestContextReductionLowersEvalTokensNoRegression`, `TestNoContextSpecificScorer` |

🚫 No `DimRetrieval` and no retrieval-specific scorer exists. Retrieval is a `rag-retrieval` context
policy with params under `DimContext` (decisions.md D-1), and its reduction is read from
`eval_tokens_total` like any other change — guarded by `TestP16AddsNoDimension` and
`TestNoContextSpecificScorer`.
