...
> An eval set reported only as a denominator is a number nobody can check. The cases themselves stay on
> the customer's machine, so the surface reports `counts_only` and names that rule rather than drawing an
> empty table that would read as a broken eval.
