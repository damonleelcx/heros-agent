...
| `p32_repo_connections` | **served** with a platform database | `GET/POST /api/v1/repo-connections`, `POST /api/v1/repo-connection-revocations`, `GET /api/v1/repo-connection-reads` — a per-repository, **read-only**, revocable grant that lets the platform clone one repository at a revision, plus the append-only ledger of every read. 🔴 Served does NOT mean anything is connected: no tenant has a grant until somebody authorizes one, and **no feature is gated on this** — a tenant who only pushes bundles reaches every surface. Revoking deletes the grant, the credential AND every tree derived from it. 🔴 **All three paths are PUBLISHED on the ingress**, so a customer's own automation can connect, read the ledger and revoke without a browser — which is what an incident needs. That makes the auth gate the first line of defence rather than the second: every one of them refuses an unauthenticated request, the tenant comes from the credential (the payload has no tenant field), and the connect body is capped and refuses unratified keys. ⚠️ The forge credential store is **in-process** by default, so a restart loses every connection's credential and each one must be re-authorized; `/readyz` reports this as `secrets_source` kind `memory-forge` |
| `p32_ingest_health` | **served** with a platform database | not a route — two documents on `GET /readyz`. `source_ingest` carries clone duration, bytes and failure cause **broken out per forge** (the aggregate is present as `total` and is never the only figure); `source_retention` carries the snapshot-retention sweep's **last successful run**, its interval and its 72-hour window. 🔴 Neither is a readiness GATE: a broken forge adapter must not take the deployment down for tenants who do not use it. Alert on `source_retention.status == "escalated"` and on `source_ingest.escalated` being non-empty |
| `p32_local_pairing` | **served** with a platform database | `POST/GET /api/v1/local-pairings` (console, in-cluster) and `POST /api/v1/local-pairing-claims` (published — `heros pair` calls it from the customer's laptop). Mode 3: the console shows a code, a person types it into a terminal that is already on the machine holding the repository, and **the repository is read there and never transmitted**. 🚫 Exactly three values cross on a claim — the code, the machine's name and a commit id — and the handler refuses a fourth field. A pairing **grants nothing**: the agent already holds the person's own credential, and no read anywhere is gated on a pairing; it only lets the console say *which machine* reads a workflow. ⚠️ Availability is stated **before** the flow starts and depends on `HEROS_PUBLIC_URL` matching the address `heros link` is pinned to; unset, the console says so rather than issuing a code that cannot be claimed |
| `p33_surface_assessment` | **served** with a platform database | `POST /api/v1/assessments` runs one, `GET /api/v1/assessments?workflow_id=…` reads the latest. **Nine axes, always all nine** — model, prompt, skills, context, tools, memory, harness, loop, graph — each in exactly one of four states: `measured`, `observed`, `not_measured`, `refused`. 🚫 **There is no overall score, grade, level or ranking**, and there is no column any of them could arrive in; what a reader gets instead is a tally — nine numbers that sum to nine. 🔴 An axis with nothing to say reports `not_measured` and **names the missing input**; it is never rendered as zero and never dropped. ⚠️ Today this is **structural only**: inference is not wired, so `memory` and `harness` report `not_measured` with `not_visible_in_static_ir` (a memory strategy lives between turns, which a static call-site extraction cannot see) and `loop` and `graph` report `refused` with cause `analysis` until P34 lands. That is the honest report for this stage, not a degraded one. 🔴 **The path is PUBLISHED on the ingress**, and `POST` **spends money on the platform's own provider account** once inference is wired — bounded by a $1.00 per-assessment cap enforced BEFORE each provider call, and by the tenant's ceiling in `heros_agent_cap` on top. Exhausting either degrades the remaining axes to `not_measured` with `budget_exhausted` and marks the report partial |
| `p33_assessment_health` | **served** with a platform database | not a route — one document on `GET /readyz`. It carries assessments **started / completed / refused**, the provider spend attributed **per tenant**, and the finding counts broken out **per axis and per state** (never an aggregate alone). 🔴 **Alert on `assessment.alerting`.** It fires when more than half of completed assessments returned NINE `not_measured` findings — which is the earliest signal that a language frontend or the sandbox has broken, and is invisible in every conventional metric: such a run completes, answers 201 and writes nine rows, so nothing errors, nothing retries and no latency moves. 🚫 It is **not** a readiness gate: a broken frontend must not pull this process out of its Service endpoints for the customers it does not affect |
| `p35_improvement_plan` | **served** with a platform database | `POST /api/v1/improvement-plans` turns a question into a **bounded plan** — the workflow and its source revision, the axes in scope, a candidate cap, a spend budget and a stopping condition — and returns it **before anything runs**. 🔴 A question that cannot be bounded is **REFUSED by name**, never run with defaults: `unbounded_requested`, `multiple_subjects`, `unknown_axis`, `no_subject`, `no_source_revision`, `no_budget`, each with a next action. Above the disclosure threshold ($1.00, the same ceiling one assessment gets) the plan must be **acknowledged by a person** before a run may begin. 🔴 **The path is PUBLISHED on the ingress** and this route **spends nothing** |
| `p35_improvement_run` | **served** with a platform database | `GET /api/v1/improvement-runs?run_id=…` reads a run back — its bound, its proposals and the **per-axis breakdown at every stage** (generated / verified / approved / delivered / withdrawn). 🔴 The breakdown lists **all nine axes**, with `in_scope` on each, so "the plan excluded this axis" and "this axis produced nothing" are not the same zero. Reconstructed from the append-only ledger rather than from a projection table, so it cannot disagree with what the reconciliation pass reads |
| `p35_improvement_run_execution` | **not mounted** anywhere | `POST /api/v1/improvement-runs` answers **503 with a named reason**. 🔴 The reason is structural, not a gap in the wiring: the P5.5 verification gate runs the **customer's eval harness**, which by design does not execute on the platform (see `internal/proposalgen`). A deployment wired with a verifier it does not have would surface **unverified** candidates, which FR8 forbids and which is the worst thing this phase could ship. This is PRD §12 **stage 1** — plan only — rather than a broken deployment, and the 503 says so |
| `p35_improvement_reconciliation` | **not mounted** anywhere | not a route — a background pass that completes deliveries interrupted between applying a change and opening its pull request, **with no human step**. 🔴 It is designed to run **every cycle rather than only after a failure**: a repair path that fires on an error flag is code that runs for the first time during an incident, on data nobody has seen. It is **not started here** for the same structural reason `p35_improvement_run_execution` is absent — this deployment cannot execute a run, so it has no interrupted deliveries to complete, and a pass ticking every five minutes to discover that would write a fresh `reconcile_last_success_ms` over a pass that examined nothing. 🚫 That would make the staleness signal lie in the one direction that matters: reporting health for a repair path that has never repaired anything |
| `p35_improvement_health` | **served** with a platform database | not a route — one document on `GET /readyz` under `improvement_run`. It carries runs **started / bounded-out / cancelled / faulted**, which bound stopped each one by name, proposals **generated / verified / approved / delivered / withdrawn broken out PER AXIS**, deliveries deduplicated, and provider spend attributed per tenant. 🔴 **Alert on `improvement_run.alerting`.** The signal underneath it is the **withdrawal rate** — a withdrawal is a change that failed to reproduce its verified delta and was stopped before it reached a repository, so every conventional metric goes the RIGHT way while it happens: the run completed, the API answered 201, nothing errored and latency did not move. 🚫 A dashboard cannot substitute: it reads historical aggregates and can look healthy while the pipeline is broken |
