// Command billing drives the whole P7 commercial stack against a REAL repository
...
//
// STUBBED — and labelled as such, exactly as cmd/proof/optimizer and cmd/proof/proposals label theirs: the DIAGNOSIS
...
//	go run ./cmd/proof/billing -repo /tmp/hermes-agent            # serve the billing surface
//	go run ./cmd/proof/billing -repo /tmp/hermes-agent -headless  # print the whole run and exit
//
...
// hermesTargets mirrors cmd/proof/optimizer's real call sites, so the two surfaces describe the same repo.
...
func main() {
	...
	if *repoFlag == "" {
		log.Fatal("p7hermes: -repo is required.\n" +
			"  git clone https://github.com/nousresearch/hermes-agent /tmp/hermes-agent\n" +
			"  go run ./cmd/proof/billing -repo /tmp/hermes-agent")
	}
...
// ── repo + spec helpers (mirroring cmd/proof/optimizer) ─────────────────────────────
