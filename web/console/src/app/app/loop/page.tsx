...
/**
 * The LOOP surface (P34, re-cut from P18's `/app/harness`).
 *
 * # What this page was, and what moved
 *
 * It was `/app/harness`, and it described a scaffold that was two things at once — "how many turns it
 * runs and in what control loop". ADR-014 split those: the ITERATION POLICY is this page, and the
 * EXECUTION ENVELOPE — sandbox posture, ceilings, retries, timeouts, guardrails — is `/app/harness`,
 * which now describes only that.
 *
 * Every item on the old page has a destination, listed in
 * `openspec/changes/p34-harness-loop-graph-split/frontend-inventory.md`. Nothing was dropped in the
 * re-cut, which is the standing failure mode of a UI revision and the one PRD §9.4 names.
 *
...
/**
 * What separates this axis from the ones it is most often confused with, in the reader's own terms.
 *
 * 🔴 FOUR rows since P34, not three. The table exists because readers conflate these axes, and P34
 * created a fourth thing to conflate — so it GREW rather than moving to whichever page won the split.
 * The last two rows are the pair this whole phase is about: how many calls happen (chosen by whoever
 * authors the loop) versus what those calls are allowed to do (imposed by whoever owns the envelope).
 */
const BOUNDARY_ROWS = [
  {
    axis: "Context",
  ...
  {
    axis: "Harness",
    scope: "AROUND the loop",
    question: "What is this node allowed to do, and inside what walls?",
    where: "Nowhere in your source — it is how the node is deployed. See /app/harness.",
  },
];
    ...
    <div className="flex flex-col gap-6">
      ...
      <Section title="Why every strategy has a ceiling">
        <Banner tone="warn" title="No strategy can express an unbounded loop">
          <p>
            🔴 <strong>There are two numbers here and they belong to two people.</strong> The{" "}
            <span className="mono">max_turns</span> below is the count <em>you choose</em>. The{" "}
            <span className="mono">turn_ceiling</span> it is checked against is a policy{" "}
            <em>imposed</em> by whoever owns this node&apos;s execution envelope, and it lives on{" "}
            <a href="/app/harness">Harness</a>. A loop asking for more than the ceiling allows is
            refused when the configuration resolves — naming both numbers, because &ldquo;too many
            turns&rdquo; leaves you unable to tell whether to lower your value or ask for a higher
            policy, and those are requests to two different people.
          </p>
          <p>
...
export default async function LoopPage() {
  ...
  const tabs: TabItem[] = [
    { id: "author", label: "Set a control loop", content: <AuthorTab /> },
    ...
    { id: "proposals", label: "Proposals", content: <ProposalsTab /> },
    {
      ...
      content: <AxisProjectionPanel axis="loop" outcome={projection} />,
    },
  ];
  ...
  return (
    ...
    >
      <p className="hint">
        This page was <span className="mono">/app/harness</span> until the axis was split. What a node
        is <em>allowed</em> to do — where it may reach, what it may spend, how long it may run — moved
        to <a href="/app/harness">Harness</a>, which imposes the ceilings this page chooses within.
      </p>
      <Tabs tabs={tabs} />
    </PageFrame>
  );
