...
Version **0.21.0**. The install script downloads the binary, checks its SHA-256 against the release manifest, checks the manifest's signature against a key pinned inside the script, and only then puts anything on your `PATH`. **Any failure is a hard stop** — there is no "continue anyway".
...
|---|---|---|---|
| `heros-0.21.0-darwin-amd64` | macOS · Intel | 17.2 MB | `ea3bb854f27b61830e7b094467ca42c516ae59116b2a32bb9c04db1c61af6e0e` |
| `heros-0.21.0-darwin-arm64` | macOS · Apple silicon | 16.7 MB | `f8cfe91d30fdb0b8b105d2f21b89f4a6d7cba096cdd7c30945fb6476d71f3dab` |
| `heros-0.21.0-linux-amd64` | Linux · x86-64 | 17.2 MB | `0d1e93e2b65688ac28ab6a9c1c24131cf80f5ffbb3e9997bd470eaf0a5d8f65a` |
| `heros-0.21.0-linux-arm64` | Linux · arm64 | 16.5 MB | `a340e232ef2f2028856db91953a939fa80f954d617f8d6630ec483808702f65a` |
| `heros-0.21.0-windows-amd64.exe` | Windows · x86-64 | 17.4 MB | `e777de688f63a4a3a77ee082a7b5897e312d5eadee184da61a79afbed030b740` |
| `install.ps1` | install script | 0.0 MB | `1df0fe2dc3fe7936c7029a878bf816f6109104d251f33fb9022d950f5e54848e` |
| `install.sh` | install script | 0.0 MB | `ce5972cc861a6a5184654f330615d1bb511e429335753fa5cfd28e2c8d1f9cd5` |

Check one by hand:

```bash
curl -fsSLO https://github.com/${OWNER}/${REPO}/releases/download/v0.21.0/SHA256SUMS
...
These are attached to release v0.21.0 but are **not listed in `SHA256SUMS`**, so the signed manifest does not cover them and neither this page nor `heros verify-release` can establish that they are ours:

| File | For |
|---|---|
| `heros_0.21.0_amd64.deb` | Debian/Ubuntu package · x86-64 |
| `heros_0.21.0_arm64.deb` | Debian/Ubuntu package · arm64 |
| `heros-0.21.0-1.aarch64.rpm` | RPM package · aarch64 |
| `heros-0.21.0-1.x86_64.rpm` | RPM package · x86-64 |
| `heros-packaging-0.21.0.tar.gz` | — |
...
curl -fsSL .../install.sh | HEROS_VERSION=0.21.0 sh
# upgrade
heros upgrade
# remove
curl -fsSL https://raw.githubusercontent.com/damonleelcx/heros-agent/v0.21.0/scripts/install.sh | HEROS_UNINSTALL=1 sh
```
...
irm https://raw.githubusercontent.com/damonleelcx/heros-agent/v0.21.0/scripts/install.ps1 | iex
# pin an exact version
$env:HEROS_VERSION='0.21.0'; irm .../install.ps1 | iex
# upgrade
heros upgrade
# remove
...
# pin an exact version
download heros_0.21.0_amd64.deb for the exact version from that release and dpkg -i it
# upgrade
sudo dpkg -i heros_0.21.0_amd64.deb   (from the newer release)
# remove
sudo dpkg -r heros
```
...
For release v0.21.0 those uncovered files are `heros_0.21.0_amd64.deb`, `heros_0.21.0_arm64.deb`.
...
# pin an exact version
rpm -i the exact version's package URL, which ends in heros-0.21.0-1.x86_64.rpm
# upgrade
sudo rpm -U heros-0.21.0-1.x86_64.rpm   (from the newer release)
# remove
sudo rpm -e heros
```
...
For release v0.21.0 those uncovered files are `heros-0.21.0-1.aarch64.rpm`, `heros-0.21.0-1.x86_64.rpm`.
...
# pin an exact version
docker run --rm ghcr.io/damonleelcx/heros:0.21.0   (or @sha256:… for a digest pin)
# upgrade
docker pull ghcr.io/damonleelcx/heros:<newer>
# remove
docker rmi ghcr.io/damonleelcx/heros:0.21.0
```
...
```bash
curl -fsSLO https://github.com/${OWNER}/${REPO}/releases/download/v0.21.0/heros-0.21.0-linux-amd64
curl -fsSLO https://github.com/${OWNER}/${REPO}/releases/download/v0.21.0/SHA256SUMS
curl -fsSLO https://github.com/${OWNER}/${REPO}/releases/download/v0.21.0/SHA256SUMS.sig
```
...
shasum -a 256 -c SHA256SUMS --ignore-missing
heros verify-release --manifest SHA256SUMS --sig SHA256SUMS.sig
install -m 0755 heros-0.21.0-linux-amd64 /usr/local/bin/heros
```
