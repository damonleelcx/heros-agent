...
Version **0.22.0**. The install script downloads the binary, checks its SHA-256 against the release manifest, checks the manifest's signature against a key pinned inside the script, and only then puts anything on your `PATH`. **Any failure is a hard stop** — there is no "continue anyway".
...
|---|---|---|---|
| `heros-0.22.0-darwin-amd64` | macOS · Intel | 20.3 MB | `a37217d752957e421a6c06574605944d55bd5a40bb35a9d84eec493b194deb79` |
| `heros-0.22.0-darwin-arm64` | macOS · Apple silicon | 19.5 MB | `4dca06048a3e19c9b34f90c47883b4157962737d986593d2bc0fc25cdab5739b` |
| `heros-0.22.0-linux-amd64` | Linux · x86-64 | 20.2 MB | `66dce942b3889bcb7990e3a85c9041825b269bfc73790e287cd06f2a40c2ae3e` |
| `heros-0.22.0-linux-arm64` | Linux · arm64 | 19.3 MB | `ab049df08670ffd77ef125a8fd36ced5d2a306200e9c1916f75bfb4f78f26f92` |
| `heros-0.22.0-windows-amd64.exe` | Windows · x86-64 | 20.6 MB | `9d88c29c68c715e96b0e8fadaf73637aec6afc677188d02e5db8f088a7fe4e44` |
| `install.ps1` | install script | 0.0 MB | `0313e5efec3d8ede0cbf2b7bb16bd5a2737acc1d871477f7204767bb5ab13ea0` |
| `install.sh` | install script | 0.0 MB | `d98d569da8c144e6ebf2712a3350e457ccb9e8aca0b07ffd616194b5ea76a08c` |

Check one by hand:

```bash
curl -fsSLO https://github.com/${OWNER}/${REPO}/releases/download/v0.22.0/SHA256SUMS
...
These are attached to release v0.22.0 but are **not listed in `SHA256SUMS`**, so the signed manifest does not cover them and neither this page nor `heros verify-release` can establish that they are ours:

| File | For |
|---|---|
| `heros_0.22.0_amd64.deb` | Debian/Ubuntu package · x86-64 |
| `heros_0.22.0_arm64.deb` | Debian/Ubuntu package · arm64 |
| `heros-0.22.0-1.aarch64.rpm` | RPM package · aarch64 |
| `heros-0.22.0-1.x86_64.rpm` | RPM package · x86-64 |
| `heros-packaging-0.22.0.tar.gz` | — |
...
curl -fsSL .../install.sh | HEROS_VERSION=0.22.0 sh
# upgrade
heros upgrade
# remove
curl -fsSL https://raw.githubusercontent.com/damonleelcx/heros-agent/v0.22.0/scripts/install.sh | HEROS_UNINSTALL=1 sh
```
...
irm https://raw.githubusercontent.com/damonleelcx/heros-agent/v0.22.0/scripts/install.ps1 | iex
# pin an exact version
$env:HEROS_VERSION='0.22.0'; irm .../install.ps1 | iex
# upgrade
heros upgrade
# remove
...
```bash
# install
curl -fsSLO https://github.com/damonleelcx/heros-agent/releases/download/v0.22.0/heros_0.22.0_amd64.deb && sudo dpkg -i heros_0.22.0_amd64.deb
# pin an exact version
download heros_0.22.0_amd64.deb for the exact version from that release and dpkg -i it
# upgrade
sudo dpkg -i heros_0.22.0_amd64.deb   (from the newer release)
# remove
sudo dpkg -r heros
```
...
For release v0.22.0 those uncovered files are `heros_0.22.0_amd64.deb`, `heros_0.22.0_arm64.deb`.
...
```bash
# install
sudo rpm -i https://github.com/damonleelcx/heros-agent/releases/download/v0.22.0/heros-0.22.0-1.x86_64.rpm
# pin an exact version
rpm -i the exact version's package URL, which ends in heros-0.22.0-1.x86_64.rpm
# upgrade
sudo rpm -U heros-0.22.0-1.x86_64.rpm   (from the newer release)
# remove
sudo rpm -e heros
```
...
For release v0.22.0 those uncovered files are `heros-0.22.0-1.aarch64.rpm`, `heros-0.22.0-1.x86_64.rpm`.
...
```bash
# install
docker run --rm -v "$PWD:/repo" ghcr.io/damonleelcx/heros:0.22.0 discover --repo /repo
# pin an exact version
docker run --rm ghcr.io/damonleelcx/heros:0.22.0   (or @sha256:… for a digest pin)
# upgrade
docker pull ghcr.io/damonleelcx/heros:<newer>
# remove
docker rmi ghcr.io/damonleelcx/heros:0.22.0
```
...
```bash
curl -fsSLO https://github.com/${OWNER}/${REPO}/releases/download/v0.22.0/heros-0.22.0-linux-amd64
curl -fsSLO https://github.com/${OWNER}/${REPO}/releases/download/v0.22.0/SHA256SUMS
curl -fsSLO https://github.com/${OWNER}/${REPO}/releases/download/v0.22.0/SHA256SUMS.sig
```
...
shasum -a 256 -c SHA256SUMS --ignore-missing
heros verify-release --manifest SHA256SUMS --sig SHA256SUMS.sig
install -m 0755 heros-0.22.0-linux-amd64 /usr/local/bin/heros
```
