...
func TestNoNewTableWasCreated(t *testing.T) {
	...
	postP26 := map[string]string{
		...
		"0026_delivery_route_mode":       "P12 — corrects 0025's delivery_route to match forgedelivery.Route: adds the required `mode` (ci|app) and drops `base_ref`, which is not a field of that type. 0025's shape could only hold routes Route.Validate rejects",
