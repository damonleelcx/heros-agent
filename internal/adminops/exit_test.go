...
func TestNoNewTableWasCreated(t *testing.T) {
	...
	postP26 := map[string]string{
		...
		"0052_p36_node_attribution":               "P36 — ONE NULLABLE column on `heros_inference`: `nodes_json`, the per-node record of what each node of the producing definition did (provider calls, tokens, latency, contribution counts, failure, skip). A COLUMN and not a table, deliberately: the grain is already covered — it sits beside `edges_json` and `labels_json`, which are the same decision made twice before — and a `heros_node_inference` table would buy per-node SQL aggregation that `jsonb_array_elements` already gives, at the cost of a second row lifecycle to keep consistent with its parent. 🔴 It is NOT the source of the per-node health endpoint's numbers: those are in-process counters, because querying this column per request would be the real-time read against the events table a CQRS split exists to prevent, and a health endpoint that goes slow when the database does reports the wrong thing at the worst moment. 🔴 NULLABLE with NO backfill and NO default, and the NULL MEANS something: every row written before the column existed has no per-node record and never will, because nobody observed which node produced those edges — a backfilled `[{\"node_id\":\"heros_analyst\"}]` would report zero calls, zero tokens and zero latency as MEASUREMENTS, the same substitution `unpriced` must not render as `0`, one table over. 🚫 It DOES NOT TOUCH `heros_agent_version.spec_json`, in either direction, ever — rewriting a stored definition into P36's nested shape would change every `config_hash` and orphan every inference pinned under the old one, and decisions.md D-36.0 establishes the rewrite is unnecessary: a single-node definition marshals to the pre-P36 bytes exactly, so an existing row reads back byte-identically with no migration at all. Idempotent two ways, one per statement class: ALTER TABLE ... ADD COLUMN IF NOT EXISTS, and INSERT ... ON CONFLICT (id) DO NOTHING. Not P26's: P26 runs no inference and creates nothing",
		"0051_p34_loop_registry":                  "P34 — the seventh content-addressed registry (`loop_entry`), holding the ITERATION POLICY a node's call runs under: which control loop, its stop condition, the turn count its AUTHOR chose, the reflection prompt, the critic binding. A new TABLE rather than a discriminator column on `harness_entry`, and the reason is the one ADR-014 is built on: the registry `Kind` is hashed into every `version_id`, which is what makes a loop ref pasted into the harness dimension fail CLOSED rather than resolving and silently binding an execution envelope where an iteration policy was asked for. Two dimensions sharing one table share one id namespace, and once customers' stored `config_hash`es reference rows in it the dimensions cannot be separated again without rewriting ids. One CREATE TABLE is the cheap side of that trade. 🔴 It ADDS NO COLUMN to any deployed table, and `harness_entry` is not touched in either direction — a migration that altered it would be ADR-014's orphaning chain arriving through the database instead of through the seal path: removing the loop fields would change every loop-bearing entry's `version_id`, hence the `config_hash` of every spec referencing it, hence make every measurement taken on a multi-turn node UNREACHABLE from any spec anyone can construct. The legacy path is therefore permanent and inert with respect to this file. 🔴 Idempotent three ways, one per statement class: CREATE TABLE IF NOT EXISTS, DROP TRIGGER IF EXISTS + CREATE (not CREATE OR REPLACE TRIGGER, which is PG14+), and INSERT ... ON CONFLICT (id) DO NOTHING. 🔴 Rollback needs no migration at all, because nothing is removed: a `loop_entry` the previous binary never reads is inert, exactly as `harness_entry` was before P18's code was enabled. Not P26's: P26 seals nothing and creates nothing",
