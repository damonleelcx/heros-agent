// Package verdictrecord is the verification VERDICT as a record — the type, its terminal gate result, and
// the two operations that keep the record self-consistent. Nothing here runs a verification.
//
// # Why this is its own package
//
// The verdict is the vocabulary two very different programs share. The platform PRODUCES one by running an
// eval; the customer's CLI READS one off disk and reports it. Before this split they shared it by both
// importing internal/verification — and that package imports the eval runner, which imports the sandbox,
// which uses Unix process groups. So `heros report-verdict`, a command that only unmarshals JSON, linked a
// process isolator, and the Windows build of the CLI stopped compiling entirely:
//
//	cmd/heros → internal/clilink → internal/verification → internal/evalrun → internal/sandbox
//
// The rule this package exists to enforce is the one internal/providercall already established for the same
// failure: a package may name a thing without linking what produces it. internal/verification aliases these
// types, so nothing that already said `verification.Verdict` had to change.

package verdictrecord
