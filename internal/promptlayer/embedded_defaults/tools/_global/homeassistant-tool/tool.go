...
func Execute(args map[string]any) (map[string]any, error) {
	...
	if url == "" {
		return toolcontract.Error("homeassistant-tool", toolcontract.ErrorCodeValidationError, "missing url", action, args), nil
	}
	...
	return toolcontract.Ok("homeassistant-tool", action, args, map[string]any{"url": url, "status_code": resp.StatusCode, "body": string(rb)}), nil
}
