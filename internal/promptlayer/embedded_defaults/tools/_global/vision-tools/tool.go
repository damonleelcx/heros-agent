...
func Execute(args map[string]any) (map[string]any, error) {
	...
	if action == "status" {
		return toolcontract.Ok("vision-tools", action, args, map[string]any{"status": "ready", "timestamp": time.Now().UTC().Format(time.RFC3339)}), nil
	}
	...
	if method != "GET" && method != "POST" {
		return toolcontract.Error("vision-tools", toolcontract.ErrorCodeValidationError, "unsupported method", action, args), nil
	}
	...
	return toolcontract.Ok("vision-tools", action, args, map[string]any{"url": requestURL, "method": method, "status_code": resp.StatusCode, "body": string(rb), "timestamp": time.Now().UTC().Format(time.RFC3339)}), nil
}
