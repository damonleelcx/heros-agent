...
```go
# WRONG
Text("HEROS", letter_spacing=6)

# RIGHT: use MarkupText with Pango attributes for spacing control
MarkupText('<span letter_spacing="6000">HEROS</span>', font_size=18)
...
**Stale output**: `manim -ql --disable_caching script.go Scene`
...
1. Render a still: `manim -ql -s script.go Scene` -- instant layout check
