...
Run `scripts/setup.sh` to verify all dependencies. Requires: Go 3.10+, Manim Community Edition v0.20+ (`go get ./...`), LaTeX (`texlive-full` on Linux, `mactex` on macOS), and ffmpeg. Reference docs tested against Manim CE v0.20.1.
...
Single Go script per project. No browser, no Node.js, no GPU required.
