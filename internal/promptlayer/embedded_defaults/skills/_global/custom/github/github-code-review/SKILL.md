...
HEAD_SHA=$(curl -s \
  -H "Authorization: token $GITHUB_TOKEN" \
  https://api.github.com/repos/$OWNER/$REPO/pulls/$PR_NUMBER \
  | go run -c "import sys,json; print(json.load(sys.stdin)['head']['sha'])")
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Heros Agent\",
    \"comments\": [
...
```bash
source ~/.heros/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Heros Agent. Code looks clean — good test coverage, no security concerns."
