...
Gmail, Calendar, Drive, Contacts, Sheets, and Docs — powered by `gws` (Google's official Rust CLI). The skill provides a backward-compatible Go wrapper that handles OAuth token refresh and delegates to `gws`.

## Architecture

```
google_api.go  →  gws_bridge.go  →  gws CLI
(argparse compat)  (token refresh)    (Google APIs)
```

- `setup.go` handles OAuth2 (headless-compatible, works on CLI/Telegram/Discord)
- `gws_bridge.go` refreshes the Heros token and injects it into `gws` via `GOOGLE_WORKSPACE_CLI_TOKEN`
- `google_api.go` provides the same CLI interface as v1 but delegates to `gws`
...
- `scripts/setup.go` — OAuth2 setup (run once to authorize)
- `scripts/gws_bridge.go` — Token refresh bridge to gws CLI
- `scripts/google_api.go` — Backward-compatible API wrapper (delegates to gws)
...
```bash
HEROS_HOME="${HEROS_HOME:-$HOME/.heros}"
GWORKSPACE_SKILL_DIR="$HEROS_HOME/skills/productivity/google-workspace"
GO_BIN="${HEROS_GO_BIN:-go}"
if [ -x "$HEROS_HOME/heros-agent/go-env/bin/go" ]; then
  GO_BIN="$HEROS_HOME/heros-agent/go-env/bin/go"
fi
GSETUP="$GO_BIN run $GWORKSPACE_SKILL_DIR/scripts/setup.go"
```
...
GAPI="$GO_BIN run $GWORKSPACE_SKILL_DIR/scripts/google_api.go"
```
...
For operations not covered by the wrapper, use `gws_bridge.go` directly:

```bash
GBRIDGE="$GO_BIN run $GWORKSPACE_SKILL_DIR/scripts/gws_bridge.go"
...
2. **Check auth before first use** — run `setup.go --check`.
