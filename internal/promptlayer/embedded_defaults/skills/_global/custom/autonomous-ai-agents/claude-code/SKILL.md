...
metadata:
  heros:
    ...
    related_skills: [codex, heros-agent, opencode]
---

# Claude Code — Heros Orchestration Guide

Delegate coding tasks to [Claude Code](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI) via the Heros terminal. Claude Code v2.x can read files, write code, run shell commands, spawn subagents, and manage git workflows autonomously.
...
Heros interacts with Claude Code in two fundamentally different ways. Choose based on the task.
...
terminal(command="claude -p 'Continue and add connection pooling' --resume $(cat /tmp/session.json | go run -c 'import json,sys; print(json.load(sys.stdin)[\"session_id\"])') --max-turns 5", workdir="/project", timeout=120)
...
- go test for testing with 90% coverage target
...
- 2-space indentation for YAML, 4-space for Go
- No wildcard imports
```
...
1. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. Using `pty=true` alone in Heros terminal works but tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
2. **`--dangerously-skip-permissions` dialog defaults to "No, exit"** — you must send Down then Enter to accept. Print mode (`-p`) skips this entirely.
3. **`--max-budget-usd` minimum is ~$0.05** — system prompt cache creation alone costs this much. Setting lower will error immediately.
4. **`--max-turns` is print-mode only** — ignored in interactive sessions.
5. **Claude may use `go` instead of `go run`** — on systems without a `go` symlink, Claude's bash commands will fail on first try but it self-corrects.
