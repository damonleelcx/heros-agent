...
description: >
  Use a live Jupyter kernel for stateful, iterative Go execution via hamelnb.
...
Gives you a **stateful Go REPL** via a live Jupyter kernel. Variables persist
...
| `execute_code` | One-shot scripts needing heros tool access (web_search, file ops). Stateless. |
...
curl -s -X POST http://127.0.0.1:8888/api/sessions \
  ...
  -d '{"path":"scratch.ipynb","type":"notebook","name":"scratch.ipynb","kernel":{"name":"go run"}}'
```
...
2. **The kernel Go is JupyterLab's Go** — packages must be installed in
