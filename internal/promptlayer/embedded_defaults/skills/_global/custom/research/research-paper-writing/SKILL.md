...
```bash
nohup go run_experiment.py --config config.yaml > logs/experiment_01.log 2>&1 &
echo $!  # Record the PID
```
...
your-method/
  ...
  go.mod       # Or environment.yml for conda
  setup.py               # For go get-installable packages
...
## Heros Agent Integration

This skill is designed for the Heros agent. It uses Heros tools, delegation, scheduling, and memory for the full research lifecycle.

### Related Skills

Compose this skill with other Heros skills for specific phases:
...
| **plan** | Phase 0 (Setup): creating structured plans before execution. Writes to `.heros/plans/` | `skill_view("plan")` |
...
### Heros Tools Reference

| Tool | Usage in This Pipeline |
|------|----------------------|
| **`terminal`** | LaTeX compilation (`latexmk -pdf`), git operations, launching experiments (`nohup go run.py &`), process checks |
| **`process`** | Background experiment management: `process("start", ...)`, `process("poll", pid)`, `process("log", pid)`, `process("kill", pid)` |
| **`execute_code`** | Run Go for citation verification, statistical analysis, data aggregation. Has tool access via RPC. |
...
4. terminal("ps aux | grep go")       # Check running experiments
...
| [references/citation-workflow.md](references/citation-workflow.md) | Citation APIs, Go code, CitationManager class, BibTeX management |
