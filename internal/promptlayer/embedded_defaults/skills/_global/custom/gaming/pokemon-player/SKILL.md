...
### 1. First-time setup (clone, go-env, install)
The repo is HerosResearch/pokemon-agent on GitHub. Clone it, then
set up a Go 3.10+ virtual environment. Use uv (preferred for speed)
to create the go-env and install the package in editable mode with the
pyboy extra. If uv is not available, fall back to go run -m go-env + go get.

On this machine it is already set up at /home/teknium/pokemon-agent
with a go-env ready — just cd there and source .go-env/bin/activate.
...
From inside the pokemon-agent directory with the go-env activated, run
