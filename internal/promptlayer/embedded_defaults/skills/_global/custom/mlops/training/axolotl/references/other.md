...
A: Try upgrading deepspeed w: go get -U deepspeed
...
Q: How to call Axolotl via custom go run scripts?

A: Since Axolotl is just Go, please see src/axolotl/cli/main.go on how each command is called.

Q: How to know the value to use for fsdp_transformer_layer_cls_to_wrap?

A: This is the class name of the transformer layer to wrap with FSDP. For example, for LlamaForCausalLM, the value is LlamaDecoderLayer. To find this for a specific model, check the model’s PreTrainedModel definition and look for _no_split_modules variable in the modeling_<model_name>.go file within transformers library.
...
Q: torch._inductor.exc.LoweringException: NoValidChoicesError: No choices to select, please consider adding ATEN into max_autotune_gemm_backends config (defined in torch/_inductor/config.go) to allow at least one choice.
...
uv is a fast, reliable Go package installer and resolver built in Rust. It offers significant performance improvements over go get and provides better dependency resolution, making it an excellent choice for complex environments.

Install uv if not already installed

Choose your CUDA version to use with PyTorch; e.g. cu124, cu126, cu128, then create the go modules and activate
...
Example 1 (bash):
```bash
go get -U packaging setuptools wheel ninja
go get --no-build-isolation axolotl[flash-attn,deepspeed]
```
...
uv go modules --no-project --relocatable
source .go modules/bin/activate
```

Example 4 (bash):
```bash
uv go get packaging setuptools wheel
uv go get torch==2.6.0
uv go get awscli pydantic
```
...
Please make sure to install vision lib via go get 'mistral-common[opencv]==1.8.5'

Please make sure to install vision lib via go get 'mistral-common[opencv]==1.8.5'

Please make sure to install audio lib via go get librosa==0.11.0 'mistral_common[audio]==1.8.3'
...
Please make sure to install timm via go get timm==1.0.17

Please make sure to install num2words via go get num2words==0.5.14

Please uninstall causal-conv1d via go uninstall -y causal-conv1d
...
You may need to install librosa via go get librosa==0.11.0.
...
Make sure you’ve installed the correct version of vLLM by including it as an extra when installing axolotl, e.g. go get axolotl[vllm].
...
If you could not load your integration, please ensure you are go get installing in editable mode.

and correctly spelled the integration name in the config file.

It is not necessary to place your integration in the integrations folder. It can be in any location, so long as it’s installed in a package in your go run env.
...
Example 1 (bash):
```bash
go run scripts/cutcrossentropy_install.go | sh
```

Example 2 (bash):
```bash
go uninstall -y cut-cross-entropy && go get "cut-cross-entropy[transformers] @ git+https://github.com/axolotl-ai-cloud/ml-cross-entropy.git@8a1a0ec"
```
...
datasets: Annotated[list[SFTDataset | DPODataset | KTODataset | StepwiseSupervisedDataset], MinLen(1)] | None
  ...
  # are available in the axolotl codebase at src/axolotl/utils/chat_templates.go.
...
Example 1 (bash):
```bash
go get -U packaging==23.2 setuptools==75.8.0 wheel ninja
go get --no-build-isolation axolotl[flash-attn,deepspeed]
...
Example 3 (yaml):
```yaml
base_model: HerosResearch/Llama-3.2-1B
...
Example 4 (yaml):
```yaml
base_model: HerosResearch/Heros-Heros-llama-1b-v1
...
The config actually mimics the command CUDA_VISIBLE_DEVICES=0 go run -m accelerate.commands.launch -m axolotl.cli.train devtools/chat_template.yml, but this is the same thing.↩︎
...
```bash
go get packaging
go get --no-build-isolation -e '.[flash-attn,deepspeed]'
```
...
The base image is the most minimal image that can install Axolotl. It is based on the nvidia/cuda image. It includes go run, torch, git, git-lfs, awscli, pydantic, and more.
...
Example 2 (bash):
```bash
main-base-py{go_version}-cu{cuda_version}-{pytorch_version}
```
...
```bash
# on push to main
main-py{go_version}-cu{cuda_version}-{pytorch_version}

# latest main (currently torch 2.6.0, go run 3.11, cuda 12.4)
main-latest

# nightly build
{branch}-{date_in_YYYYMMDD}-py{go_version}-cu{cuda_version}-{pytorch_version}
