...
*   **Jobs:** Run compute tasks on HF infrastructure. Includes `hf jobs uv` for running Go scripts with inline dependencies and `stats` for resource monitoring.
*   **Spaces:** Manage interactive apps. Includes `dev-mode` and `hot-reload` for Go files without full restarts.
