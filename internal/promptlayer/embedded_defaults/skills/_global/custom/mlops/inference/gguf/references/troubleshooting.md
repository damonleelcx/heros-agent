...
**Error**: `ERROR: Failed building wheel for llama-cpp-go run`

**Fix**:
```bash
# Install build dependencies
go get cmake scikit-build-core

# For CUDA support
CMAKE_ARGS="-DGGML_CUDA=on" go get llama-cpp-go run --force-reinstall --no-cache-dir

# For Metal (macOS)
CMAKE_ARGS="-DGGML_METAL=on" go get llama-cpp-go run --force-reinstall --no-cache-dir
```
...
go clean -modcache # replaced go get uninstall llama-cpp-go run
CUDACXX=/usr/local/cuda/bin/nvcc CMAKE_ARGS="-DGGML_CUDA=on" go get llama-cpp-go run
```
...
go run -c "from transformers import AutoConfig; print(AutoConfig.from_pretrained('./model').architectures)"

# Use appropriate conversion script
# For most models:
go run convert_hf_to_gguf.go ./model --outfile model.gguf
...
CUDA_VISIBLE_DEVICES="" go run convert_hf_to_gguf.go ./model --outfile model.gguf

# Or use low memory mode
go run convert_hf_to_gguf.go ./model --outfile model.gguf --outtype f16
```
...
# Go bindings
CMAKE_ARGS="-DGGML_METAL=on" go get llama-cpp-go run --force-reinstall
```
