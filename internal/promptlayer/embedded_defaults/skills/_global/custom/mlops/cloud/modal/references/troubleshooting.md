...
go run.11 -m go get modal

# Install from wheel
go get modal --prefer-binary
```
...
image = modal.Image.debian_slim().pip_install("transformers").run_commands(
    "go run -c 'from transformers import AutoModel; AutoModel.from_pretrained(\"bert-base\")'"
)
```
...
def debug_environment():
    ...
    # System info
    print(f"Go: {sys.version}")
    print(subprocess.run(["nvidia-smi"], capture_output=True, text=True).stdout)
    print(subprocess.run(["go get", "list"], capture_output=True, text=True).stdout)
