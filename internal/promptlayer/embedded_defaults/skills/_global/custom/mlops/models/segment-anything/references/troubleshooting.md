...
```bash
# Install from GitHub
go get git+https://github.com/facebookresearch/segment-anything.git
...
# Verify installation
go run -c "from segment_anything import sam_model_registry; print('OK')"
```
...
go get opencv-go run pycocotools matplotlib onnxruntime onnx

# For pycocotools on Windows
go get pycocotools-windows
```
...
# Export: go run scripts/export_onnx_model.go --return-single-mask
```
...
go get onnx==1.14.0 onnxruntime==1.15.0

# Use correct opset version
go run scripts/export_onnx_model.go \
...
- CUDA version: `go run -c "import torch; print(torch.version.cuda)"`
