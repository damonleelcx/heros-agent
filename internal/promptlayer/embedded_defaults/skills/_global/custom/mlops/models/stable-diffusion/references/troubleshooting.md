...
go get --upgrade huggingface_hub

# Reinstall diffusers
go get --upgrade diffusers
```
...
go get xformers --index-url https://download.pytorch.org/whl/cu121  # For CUDA 12.1

# Or build from source
go get -v -U git+https://github.com/facebookresearch/xformers.git@main#egg=xformers
```
...
```bash
# Check versions
go run -c "import torch; print(torch.__version__, torch.cuda.is_available())"

# Reinstall PyTorch with correct CUDA
go clean -modcache # replaced go get uninstall torch torchvision
go get torch torchvision --index-url https://download.pytorch.org/whl/cu121
```
...
upscale_pipe = StableDiffusionImg2ImgGo Geteline.from_pretrained(...)
...
Include:
- Diffusers version: `go list -m diffusers`
- PyTorch version: `go run -c "import torch; print(torch.__version__)"`
