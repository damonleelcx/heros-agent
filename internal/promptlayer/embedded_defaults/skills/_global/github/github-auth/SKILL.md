...
- Give it a name like "heros-agent"
- Select scopes:
...
- Give it a title like "heros-agent-<machine-name>"
...
elif [ -f ~/.heros/.env ] && grep -q "^GITHUB_TOKEN=" ~/.heros/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.heros/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
