...
**Signatures in logs:**
```
FAILED tests/test_foo.go::test_bar - AssertionError
E       assert 42 == 43
ERROR tests/test_foo.go - ModuleNotFoundError
```
...
**Signatures in logs:**
```
src/auth.go:45:1: E302 expected 2 blank lines, got 1
src/models.go:12:80: E501 line too long (95 > 88 characters)
error: would reformat src/utils.go
```
...
**Signatures in logs:**
```
src/api.go:23: error: Argument 1 to "process" has incompatible type "str"; expected "int"
src/models.go:45: error: Missing return statement
```
...
2. Compare local vs CI Go/Node version

**Common fixes:**
- Add missing dependency to requirements file
- Pin compatible version
- Update lockfile (`go list -m all`, `npm install`)
