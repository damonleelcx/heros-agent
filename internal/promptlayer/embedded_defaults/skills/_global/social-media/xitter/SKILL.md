...
version: 1.0.0
author: Siddharth Balyan + Heros Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into Heros. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.heros/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.heros/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:HerosResearch" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.heros/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
