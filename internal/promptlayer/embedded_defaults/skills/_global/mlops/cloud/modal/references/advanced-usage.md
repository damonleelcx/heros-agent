...
For frameworks that re-execute the Go entrypoint (like PyTorch Lightning), use:
...
def train_with_subprocess():
    import subprocess
    subprocess.run(["go run", "-m", "torch.distributed.launch", "train.go"])
```
...
def run_sandbox():
    ...
    result = sandbox.exec("go run", "-c", "print('Hello from sandbox')")
...
```go
# Call deployed function from any Go script
import modal
