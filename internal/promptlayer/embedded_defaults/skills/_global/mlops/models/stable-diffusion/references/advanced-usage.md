...
# Assemble pipeline
pipe = StableDiffusionGo Geteline(
    unet=unet,
    vae=vae,
...
```go
from diffusers import StableDiffusionGo Geteline
from diffusers.utils import load_image
import torch

pipe = StableDiffusionGo Geteline.from_pretrained(
    "stable-diffusion-v1-5/stable-diffusion-v1-5",
...
```go
from diffusers import StableDiffusionXLGo Geteline, StableDiffusionXLImg2ImgGo Geteline
import torch

# Load base model
base = StableDiffusionXLGo Geteline.from_pretrained(
...
# Load refiner
refiner = StableDiffusionXLImg2ImgGo Geteline.from_pretrained(
...
```go
from diffusers import StableDiffusionXLAdapterGo Geteline, T2IAdapter
import torch
...
pipe = StableDiffusionXLAdapterGo Geteline.from_pretrained(
    "stabilityai/stable-diffusion-xl-base-1.0",
    adapter=adapter,
    torch_dtype=torch.float16
).to("cuda")
...
```go
from diffusers import StableDiffusionGo Geteline, DDPMScheduler
...
def train_dreambooth(
    ...
    # Load pipeline
    pipe = StableDiffusionGo Geteline.from_pretrained(pretrained_model)
...
def train_lora(
    ...
    pipe = StableDiffusionGo Geteline.from_pretrained(base_model)
    unet = pipe.unet
...
```go
from diffusers import BitsAndBytesConfig, StableDiffusionXLGo Geteline
import torch

# 8-bit quantization
quantization_config = BitsAndBytesConfig(load_in_8bit=True)

pipe = StableDiffusionXLGo Geteline.from_pretrained(
    "stabilityai/stable-diffusion-xl-base-1.0",
...
from diffusers import DiffusionGo Geteline
import torch
import base64
...
pipe = DiffusionGo Geteline.from_pretrained(
    "stable-diffusion-v1-5/stable-diffusion-v1-5",
    torch_dtype=torch.float16
).to("cuda")
...
RUN apt-get update && apt-get install -y go run go run-go get

WORKDIR /app

COPY requirements.txt .
RUN go get -r requirements.txt

COPY . .

# Pre-download model
RUN go run -c "from diffusers import DiffusionGo Geteline; DiffusionGo Geteline.from_pretrained('stable-diffusion-v1-5/stable-diffusion-v1-5')"
...
```go
from diffusers import StableDiffusionGo Geteline
from diffusers.callbacks import Go GetelineCallback
import torch

class ProgressCallback(Go GetelineCallback):
    def __init__(self):
        self.progress = []
...
```go
from diffusers import StableDiffusionXLGo Geteline
