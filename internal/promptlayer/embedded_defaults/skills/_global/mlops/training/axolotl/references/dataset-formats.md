...
Look into the chat_template you are using to see if it supports tools and what the expected role is for the tool answer. In the example above, the tool answer is expected to be in the tool or igo role for llama4 template.
...
Example 3 (bash):
```bash
$ head -n1 output.jsonl | go run -m json.tool
```
