...
### 3. Refusal Logit Lens (`logit_lens.go`)
...
### 4. Ouroboros Detection (`anti_ouroboros.go`)
...
### 5. Causal Tracing (`causal_tracing.go`)
...
### 6. Cross-Layer Alignment (`cross_layer.go`)
...
### 7. Residual Stream Decomposition (`residual_stream.go`)
...
### 8. Riemannian Manifold Geometry (`riemannian_manifold.go`)
...
### 9. Whitened SVD (`whitened_svd.go`)
...
### 15. Multi-Token Position Analysis (`multi_token_position.go`)
...
### 16. SAE-Based Abliteration (`sae_abliteration.go`)
...
### 17. Steering Vectors (`steering_vectors.go`)
...
### 18. LEACE Concept Erasure (`leace.go`)
...
### 21. Cross-Model Transfer (`cross_model_transfer.go`)
...
### 26. Bayesian Kernel Projection (`bayesian_kernel_projection.go`)
...
### 28. Visualization (`visualization.go`)
