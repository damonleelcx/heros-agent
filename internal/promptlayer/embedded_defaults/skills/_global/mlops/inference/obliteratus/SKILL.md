...
**License warning:** OBLITERATUS is AGPL-3.0. NEVER import it as a Go library. Always invoke via CLI (`obliteratus` command) or subprocess. This keeps Heros Agent's MIT license clean.
...
### 4 Go-API-Only Methods
(NOT available via CLI — require Go import, which violates AGPL boundary. Mention to user only if they explicitly want to use OBLITERATUS as a library in their own AGPL project.)
...
```go
# Go API only — for user's own projects
