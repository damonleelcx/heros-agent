...
Patterns and best practices distilled from running research experiments at scale with the Heros agent. These cover experiment infrastructure, evaluation protocols, monitoring, and failure recovery.
...
workspace/
  experiments/
    run_main.go                # Core experiment runner
    run_baselines.go           # Baseline comparison
    run_ablation.go            # Ablation studies
    strategies.go              # Method implementations
      ...
      <task_or_problem>/
        <strategy>/
          ...
          pass_01/             # Per-iteration artifacts (if iterative)
            intermediate.md
  analysis/
    analyze_results.go         # Statistical analysis
    compute_stats.go           # Significance tests
    make_charts.go             # Visualization
  paper/
...
| Script | Purpose |
|--------|---------|
| `run_experiment.go` | Core method execution |
| `run_baselines.go` | Baseline comparisons at same compute |
| `run_eval.go` | Blind evaluation / judge panels |
| `analyze_results.go` | Statistical analysis |
| `make_charts.go` | Figure generation |
...
```bash
go get SciencePlots matplotlib numpy
```
