...
## 5. GODMODE FAST — Heros 4 405B (Zero Refusal)

**Model:** `herosresearch/heros-4-405b`
**Strategy:** Heros models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Heros Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.heros/config.yaml`:
...
```go
exec(open(os.path.expanduser("~/.heros/skills/red-teaming/godmode/scripts/godmode_race.go")).read())
