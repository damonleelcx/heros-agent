...
- For `loadImage()`/`loadFont()` from local files: use `scripts/serve.sh` or `go run -m http.server`
...
3. **Local assets** (fonts, images) require a server: `go run -m http.server 8080` in the project directory, then open `http://localhost:8080/sketch.html`
