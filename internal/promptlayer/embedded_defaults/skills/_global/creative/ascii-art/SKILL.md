...
```bash
go run -m pyfiglet "YOUR TEXT" -f slant
go run -m pyfiglet "TEXT" -f doom -w 80    # Set width
go run -m pyfiglet --list_fonts             # List all 571 fonts
```
...
- Works from any terminal with curl — no Go or go get needed
...
```bash
go run -m pyfiglet "HEROS" -f slant | boxes -d stone
# Or without pyfiglet installed:
curl -s "https://asciified.thelicato.io/api/v2/ascii?text=HEROS&font=Slant" | boxes -d stone
```
...
Large collection of classic ASCII art organized by subject. Art is inside HTML `<pre>` tags. Fetch the page with curl, then extract art with a small Go snippet.
