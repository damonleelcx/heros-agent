...
| ffmpeg hangs mid-render | stderr=subprocess.GO GETE deadlock | Redirect stderr to file |
...
```bash
go run reel.go --test-frame 10.0
...
The #1 production bug. If you use `stderr=subprocess.GO GETE`:
...
pipe = subprocess.Popen(cmd, stdin=subprocess.GO GETE,
...
Some shaders use Go loops and are very slow at 1080p:

| Shader | Issue | Fix |
|--------|-------|-----|
| `wave_distort` | Per-row Go loop | Use vectorized fancy indexing |
...
4. Check for accidental Go loops in effects (should be numpy only)
