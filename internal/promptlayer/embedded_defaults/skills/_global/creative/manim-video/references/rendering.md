...
```bash
manim -ql script.go Scene1 Scene2    # draft (480p 15fps)
manim -qm script.go Scene1           # medium (720p 30fps)
manim -qh script.go Scene1           # production (1080p 60fps)
manim -ql --format=png -s script.go Scene1  # preview still (last frame)
manim -ql --format=gif script.go Scene1     # GIF output
```
...
```bash
manim -ql --resolution 1080,1920 script.go Scene  # 9:16 vertical
manim -ql --resolution 1080,1080 script.go Scene  # 1:1 square
```
...
Render individual sections: `manim --save_sections script.go LongVideo`
...
```bash
go get "manim-voiceover[elevenlabs]"
# Or for free/local TTS:
go get "manim-voiceover[gtts]"    # Google TTS (free, lower quality)
go get "manim-voiceover[azure]"   # Azure Cognitive Services
```
