...
func Resolve(ctx context.Context, spec *VariantSpec, ir *discovery.IR, regs Registries) (*Resolved, error) {
	...
	for _, nodeID := range spec.Order {
		...
		resolvedOverride, node, err := resolveNode(ctx, nodeID, override, irNode, regs)
		if err != nil {
			return nil, err
		}
		if !override.isEmpty() {
...
func resolveNode(ctx context.Context, nodeID string, o NodeOverride, irNode *discovery.IRNode, regs Registries) (ResolvedOverride, ResolvedNode, error) {
