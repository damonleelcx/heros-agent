...
func TestPG_ZZ_DownMigrationRemovesVariantSpecOnly(t *testing.T) {
	...
	if err := testDB.QueryRowContext(ctx,
		...
		// FALSE GREEN: 0003 could drop the function in this schema and the test would still pass,
		// because somebody else's exists. (The `to_regclass` check a few lines above is already
		// search_path-scoped, which is why it never had this problem.)
		`SELECT count(*) > 0 FROM pg_proc
		  WHERE proname = 'registry_reject_mutation'
		    AND pronamespace = current_schema()::regnamespace`).Scan(&fnExists); err != nil {
