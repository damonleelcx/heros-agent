...
// BillingSource is what the API asks of the billing capability: the whole billing/usage read model for one
...
// MountBilling registers the billing/usage UI, its read endpoint, and the consent actions.
func (s *Server) MountBilling(src BillingSource) {
	s.billingView = src
	s.Mux.HandleFunc("GET /billing", s.handleBillingUI)
	s.Mux.HandleFunc("GET /api/v1/customers/{customer_id}/billing", s.handleBilling)
	s.Mux.HandleFunc("POST /api/v1/customers/{customer_id}/gainshare-consent", s.handleGainshareConsent)
}

func (s *Server) handleBillingUI(w http.ResponseWriter, _ *http.Request) {
...
func (s *Server) handleGainshareConsent(w http.ResponseWriter, r *http.Request) {
	if s.billingView == nil {
		writeJSON(w, http.StatusServiceUnavailable, map[string]string{"error": "the p7 billing surface is not mounted on this server"})
		return
	}
	...
	v, err := s.billingView.SetGainshareConsent(r.PathValue("customer_id"), body.Consented)
	if err != nil {
