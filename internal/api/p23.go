...
// P23Source is the consent dependency.
type P23Source interface {
...
func (s *Server) handleP23Record(w http.ResponseWriter, r *http.Request) {
	...
	stored, created, err := s.p23.Record(r.Context(), principal.TenantID, principalIDOf(principal), legal.Request{
...
func (s *Server) handleP23Read(w http.ResponseWriter, r *http.Request) {
	principal, ok := auth.PrincipalFrom(r.Context())
	if !ok {
		writeJSON(w, http.StatusUnauthorized, map[string]string{"error": "no session"})
		return
	...
	history, err := s.p23.Read(r.Context(), principal.TenantID, principalIDOf(principal))
