...
// P13AuthoringSource is the authoring surface this server mounts. It is an interface for the reason
// every other mount is: the server needs the ANSWERS, not the packages that compute them.
type P13AuthoringSource interface {
...
func (s *Server) handleAuthoringPreflight(w http.ResponseWriter, r *http.Request) {
	...
	res, err := s.p13authoring.Preflight(r.Context(), draft)
	if err != nil {
...
func (s *Server) handleAuthoringSubmit(w http.ResponseWriter, r *http.Request) {
	...
	sub, err := s.p13authoring.Submit(r.Context(), draft)
	if err != nil {
		s.writeAuthoringError(w, err)
		return
	}
...
func (s *Server) handleAuthoringRevert(w http.ResponseWriter, r *http.Request) {
	...
	rev, err := s.p13authoring.Revert(r.Context(), req.ChangeID, actor)
	if err != nil {
		s.writeAuthoringError(w, err)
		return
	}
...
func (s *Server) handleAuthoringHistory(w http.ResponseWriter, r *http.Request) {
	...
	entries, err := s.p13authoring.History(r.Context(), actor.TenantID, changeID)
	if err != nil {
		s.writeAuthoringError(w, err)
		return
	}
