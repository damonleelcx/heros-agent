...
// P10Store is the read/write surface the prompt-authoring routes need. *registry.Store satisfies it;
// an interface so these handlers can be tested without Postgres.
type P10Store interface {
...
// MountP10 registers the prompt-authoring routes. Call after New. Optional, like the other mounts:
...
func (s *Server) handlePublishPrompt(w http.ResponseWriter, r *http.Request) {
	...
	versionID, err := s.p10.RegisterPrompt(r.Context(), scopedName(principal.TenantID, req.Name), req.Body)
	if err != nil {
...
func (s *Server) handlePromptNames(w http.ResponseWriter, r *http.Request) {
	...
	names, err := s.p10.PromptNames(r.Context(), "t:"+principal.TenantID+"/")
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, specError{Error: err.Error()})
		return
	}
...
func (s *Server) handlePromptTimeline(w http.ResponseWriter, r *http.Request) {
	...
	entries, err := s.p10.PromptTimeline(r.Context(), scopedName(principal.TenantID, name))
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, specError{Error: err.Error()})
		return
	}
...
func (s *Server) handlePromptDiff(w http.ResponseWriter, r *http.Request) {
	...
	diff, err := s.p10.DiffPromptVersions(r.Context(), a, b)
	if err != nil {
...
func (s *Server) handleStudioPreview(w http.ResponseWriter, r *http.Request) {
	...
	rendered, err := s.p10.StudioRender(r.Context(), req.VersionID, req.Bindings)
	if err != nil {
		if errors.Is(err, registry.ErrNotFound) {
			writeJSON(w, http.StatusNotFound, specError{Error: err.Error()})
			return
