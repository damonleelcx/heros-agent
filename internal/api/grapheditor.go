...
func (s *Server) MountGraphEditor(src GraphEditorSource) {
	s.graphEditor = src
	s.Mux.HandleFunc("GET /editor", s.handleGraphEditorUI)
	s.Mux.HandleFunc("GET /api/v1/workflows/{workflow_id}/ir", s.handleGraphIR)
	s.Mux.HandleFunc("POST /api/v1/workflows/{workflow_id}/validate", s.handleGraphValidate)
	s.Mux.HandleFunc("POST /api/v1/workflows/{workflow_id}/commit", s.handleGraphCommit)
	s.Mux.HandleFunc("POST /api/v1/workflows/{workflow_id}/orderings", s.handleGraphOrderings)
	s.Mux.HandleFunc("POST /api/v1/workflows/{workflow_id}/orderings/stream", s.handleGraphOrderingsStream)
}

func (s *Server) handleGraphEditorUI(w http.ResponseWriter, _ *http.Request) {
...
// handleGraphOrderings enumerates every ordering of the workflow's nodes, validates and scores each, and
...
// handleGraphOrderingsStream streams arrangements as they are DISCOVERED (task: live discovery animation).
