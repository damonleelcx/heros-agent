...
// MountPayments registers the collection surface — P21 tasks 6.1 and 6.2.
func (s *Server) MountPayments(src PaymentsSource) {
	s.payments = src
	s.Mux.HandleFunc("GET /api/v1/customers/{customer_id}/payment", s.handlePayment)
	s.Mux.HandleFunc("POST /api/v1/customers/{customer_id}/checkout-session", s.handleCheckout)
	s.Mux.HandleFunc("POST /api/v1/customers/{customer_id}/plan", s.handlePlan)
}
...
func (s *Server) handlePayment(w http.ResponseWriter, r *http.Request) {
	...
	v, ok := s.payments.Payment(customerID, r.URL.Query().Get("period"))
	if !ok {
...
func (s *Server) handleCheckout(w http.ResponseWriter, r *http.Request) {
	...
	v, err := s.payments.StartCheckout(r.Context(), r.PathValue("customer_id"), body.PlanName, body.SuccessURL, body.CancelURL)
	if err != nil {
...
func (s *Server) handlePlan(w http.ResponseWriter, r *http.Request) {
	...
	v, err := s.payments.ChangePlan(r.Context(), r.PathValue("customer_id"), body.PlanName)
	if err != nil {
		writeJSON(w, http.StatusUnprocessableEntity, map[string]string{"error": err.Error()})
		return
	}
