...
// fakeP7 is a canned BillingSource.
type fakeP7 struct {
	view       BillingView
	found      bool
	consent    bool
...
func newP7Server(src BillingSource) *Server {
...
func TestP7UnmountedReturns503(t *testing.T) {
	s := New(nil, config.Config{})
	s.MountBilling(nil) // routes registered, source absent — the deployment-without-billing shape
