...
//
// It was written after noticing that `RegisterConsent` had **no caller anywhere** and no test — so
// `handleConsentRecord` and `handleConsentRead` had never been executed. The domain underneath was thoroughly
...
func consentServer(t *testing.T, src ConsentSource) *Server {
	t.Helper()
	s := New(nil, config.Config{})
	s.RegisterConsent(src)
	return s
}
...
func TestP23_AnUnmountedDeploymentSaysSoRatherThanPanicking(t *testing.T) {
	// A deployment that never called RegisterConsent has no consent surface. It must answer, not crash — and
