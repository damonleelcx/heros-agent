...
// ForgeDeliverySource is the P12 dependency: the forge-delivery service.
type ForgeDeliverySource interface {
...
// MountForgeDelivery registers the delivery routes. Optional, like every other mount.
func (s *Server) MountForgeDelivery(src ForgeDeliverySource) {
	s.forgeDelivery = src
	s.Mux.HandleFunc("GET /api/v1/deliveries", s.handleDeliveries)
	s.Mux.HandleFunc("GET /api/v1/ci/pending", s.handleCIPending)
	s.Mux.HandleFunc("POST /api/v1/ci/report", s.handleCIReport)
}

func (s *Server) forgeDeliveryTenant(w http.ResponseWriter, r *http.Request) (string, bool) {
...
// handleDeliveries serves the console delivery read model (tasks 6.1–6.3).
func (s *Server) handleDeliveries(w http.ResponseWriter, r *http.Request) {
	...
	tenant, ok := s.forgeDeliveryTenant(w, r)
	if !ok {
		return
	}
	heads, err := s.forgeDelivery.ListDeliveries(r.Context(), tenant)
	if err != nil {
		writeJSON(w, http.StatusBadGateway, specError{Error: "reading deliveries: " + err.Error()})
		return
	}
	cond, err := s.forgeDelivery.RouteConditionFor(r.Context(), tenant)
	if err != nil {
...
// handleCIPending serves the authenticated CI fetch: verified, deliverable Prepared proposals for a
// target, scoped server-side to the caller's tenant (task 5.3). The target comes from the query; the
// tenant NEVER does.
func (s *Server) handleCIPending(w http.ResponseWriter, r *http.Request) {
	if s.forgeDelivery == nil {
		writeJSON(w, http.StatusServiceUnavailable, specError{Error: "forge delivery is not mounted on this server"})
		return
	}
...
// handleCIReport records a CI-opened delivery (task 5.1 report leg).
func (s *Server) handleCIReport(w http.ResponseWriter, r *http.Request) {
	if s.forgeDelivery == nil {
		writeJSON(w, http.StatusServiceUnavailable, specError{Error: "forge delivery is not mounted on this server"})
		return
	}
	...
	res, err := s.forgeDelivery.RecordReport(r.Context(), tenant, prep, report)
	if err != nil {
