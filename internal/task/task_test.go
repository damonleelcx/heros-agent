...
func TestApprovalDoesNotHoldALease(t *testing.T) {
	...
	// 🔴 → Ready rather than → Running: an approved task rejoins the claimable pool, and claiming is
	// what acquires the lease. Straight to Running would execute with no lease and no owner, invisible
	// to the reclaim path that exists to rescue exactly that.
	if !AwaitingApproval.CanTransitionTo(Ready) {
		t.Fatal("an approved task cannot rejoin the claimable pool")
	}
	if AwaitingApproval.CanTransitionTo(Running) {
		t.Fatal("a parked task can move straight to running, skipping the claim that gives it a lease")
	}
...
// TestAFailedContributorDegradesRatherThanBlocks.
//
// 🔴 Regression fence for a bug that shipped. An eval set is generated by four independent strategies,
// one of which fails honestly when no trace source is connected. With every generator REQUIRED, that
// failure blocked the quality gate, blocked publication, and a run holding three generators' worth of
// good cases delivered nothing.
//
// The gate needs its generators FINISHED, not SUCCESSFUL — different questions the graph could only ask
// one of.
func TestAFailedContributorDegradesRatherThanBlocks(t *testing.T) {
	gate := tk("gate")
	gate.Contributes = []ID{"a", "b"}
	d := mustDAG(t, tk("a"), tk("b"), gate)

	if len(d.ReadySet()) != 2 {
		t.Fatalf("%d ready; the gate must wait for its contributors", len(d.ReadySet()))
	}
	d.Tasks["a"].State = Succeeded
	if len(d.ReadySet()) != 1 {
		t.Fatal("the gate became ready with a contributor still running")
	}
	d.Tasks["b"].State = Failed
	d.PropagateFailure()

	if got := d.Tasks["gate"].State; got == Blocked {
		t.Fatal("a failed CONTRIBUTOR blocked the gate; three generators' worth of cases would be lost " +
			"because a fourth had nothing to offer")
	}
	ready := d.ReadySet()
	if len(ready) != 1 || ready[0].ID != "gate" {
		t.Fatalf("the gate is not runnable once every contributor is terminal; ready=%v", ready)
	}
}

// TestAFailedREQUIREMENTStillBlocks. The new kind of edge must not weaken the old one.
func TestAFailedRequirementStillBlocks(t *testing.T) {
	consumer := tk("consumer", "required")
	consumer.Contributes = []ID{"optional"}
	d := mustDAG(t, tk("required"), tk("optional"), consumer)

	d.Tasks["required"].State = Failed
	d.Tasks["optional"].State = Succeeded
	d.PropagateFailure()

	if got := d.Tasks["consumer"].State; got != Blocked {
		t.Fatalf("consumer is %s after its REQUIRED dependency failed", got)
	}
}

// TestContributoryEdgesAreValidatedAndCycleChecked. A contributory edge is still an edge.
func TestContributoryEdgesAreValidatedAndCycleChecked(t *testing.T) {
	dangling := tk("x")
	dangling.Contributes = []ID{"nope"}
	if _, err := NewDAG("g1", []*Task{dangling}); !errors.Is(err, ErrUnknownTask) {
		t.Errorf("a contributory edge to nowhere was accepted: %v", err)
	}
	a, b := tk("a"), tk("b")
	a.Contributes = []ID{"b"}
	b.Contributes = []ID{"a"}
	if _, err := NewDAG("g1", []*Task{a, b}); !errors.Is(err, ErrCycle) {
		t.Errorf("a cycle through contributory edges was accepted: %v", err)
	}
}
