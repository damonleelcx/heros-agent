...
var transitions = map[State][]State{
	...
	// 🔴 → Ready, not → Running. An approved task rejoins the CLAIMABLE pool; claiming is what acquires
	// the lease, and a task moved straight to Running would be executing with no lease and no owner —
	// invisible to the reclaim path that exists to rescue exactly that.
	AwaitingApproval: {Ready, Cancelled, Failed},
	Succeeded:        {},
	Failed:           {},
	Blocked:          {},
	Cancelled:        {},
...
type Task struct {
	...
	DependsOn []ID
	// Contributes are dependencies whose results are USED IF AVAILABLE. They must all be terminal before
	// this task runs, but they need not have succeeded.
	//
	// # 🔴 Why this distinction exists, and the bug that produced it
	//
	// An eval set is generated by four independent strategies, each blind to what the others find. One
	// of them — seeding from real traces — fails honestly when no trace source is connected. With only
	// DependsOn, that failure blocked the quality gate, which blocked publication, and a run that had
	// produced three generators' worth of good cases delivered nothing.
	//
	// The gate needed its generators FINISHED, not SUCCESSFUL. Those are different questions and the
	// graph could only ask one of them. The artefact already had a `Missing` field to record which
	// strategies contributed nothing; the DAG never let it reach the case.
	//
	// 🚫 Not the same as "optional". A contributory dependency is still waited for — running the gate
	// while a generator is mid-flight would silently omit its cases, which is exactly the shape of
	// incompleteness this system exists to make visible.
	Contributes []ID
	State       State
	...
	// Approved records that a person answered this task's approval gate.
	//
	// 🔴 Distinct from the POLICY, which answers "does this kind of work need a person?" — a property of
	// the task that never changes. Whether somebody has already answered is a different fact, and
	// without it the gate re-fires on every claim: park, approve, claim, park, forever. The run never
	// progresses and each re-approval looks successful, because the task genuinely is awaiting approval
...
func NewDAG(goalID string, tasks []*Task) (*DAG, error) {
	...
	for _, t := range tasks {
		for _, dep := range append(append([]ID(nil), t.DependsOn...), t.Contributes...) {
...
func (d *DAG) detectCycle() error {
	...
	for _, id := range d.ids() {
		...
		for len(stack) > 0 {
			...
			case white:
				colour[cur] = grey
				for _, dep := range d.Tasks[cur].edges() {
					switch colour[dep] {
					case grey:
...
// edges returns every dependency, required and contributory. Used where the question is structural —
// cycles, unknown references — rather than about outcomes.
func (t *Task) edges() []ID {
	if len(t.Contributes) == 0 {
		return t.DependsOn
	}
	return append(append([]ID(nil), t.DependsOn...), t.Contributes...)
}

// dependenciesSucceeded reports whether this task may run: every REQUIRED dependency succeeded, and
// every CONTRIBUTORY one is terminal.
func (d *DAG) dependenciesSucceeded(t *Task) bool {
	...
	for _, dep := range t.Contributes {
		if !d.Tasks[dep].State.Terminal() {
			return false
		}
	}
	return true
...
func (d *DAG) PropagateFailure() int {
	...
	for changed := true; changed; {
		...
		for _, id := range d.ids() {
			...
			// 🔴 Only REQUIRED dependencies block. A contributory dependency that failed is a gap in the
			// result, recorded by the task that consumes it, not a reason to abandon the branch.
			for _, dep := range t.DependsOn {
