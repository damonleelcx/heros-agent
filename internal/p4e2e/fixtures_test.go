// Package p4e2e is the P4 end-to-end proof: one in-process pipeline from IR to leaderboard, with
...
func fxIR() *discovery.IR {
	...
	return &discovery.IR{
		...
		Workflow:  discovery.IRWorkflow{ID: "wf-p4e2e", Language: "python"},
...
func fxHandAuthored() []evalharness.Case {
	mk := func(id, route, q string, turns int) evalharness.Case {
		...
		return evalharness.Case{
			CaseID: id, WorkflowID: "wf-p4e2e", Suite: "hand",
