...
// stripefake_test.go is the Stripe the P21 tests actually talk to.
//
...
const (
	testStripeKey = "sk_test_p21_fake_key_not_a_secret"
	liveStripeKey = "sk_live_p21_fake_key_not_a_secret"
)

// fakeStripe is an in-process Stripe.
...
func (f *fakeStripe) URL() string { return f.srv.URL }

func (f *fakeStripe) SetDown(v bool) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.down = v
}

func (f *fakeStripe) SetFailAfterRecord(v bool) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.failAfterRecord = v
}

func (f *fakeStripe) SetRequireLiveKey(v bool) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.requireLiveKey = v
}

func (f *fakeStripe) Calls(op string) int {
	f.mu.Lock()
	defer f.mu.Unlock()
	return f.calls[op]
}
...
// asserts on.
func (f *fakeStripe) ItemCount() int {
	f.mu.Lock()
	defer f.mu.Unlock()
	return len(f.items)
}

func (f *fakeStripe) CreditCount() int {
	f.mu.Lock()
	defer f.mu.Unlock()
	return len(f.credits) + len(f.refunds)
}

func (f *fakeStripe) IdempotencyKeys() []string {
	f.mu.Lock()
	defer f.mu.Unlock()
	return append([]string(nil), f.seenIdem...)
}

func (f *fakeStripe) AuthHeaders() []string {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeStripe) Item(id string) (fakeItem, bool) {
	f.mu.Lock()
	defer f.mu.Unlock()
	it, ok := f.items[id]
	if !ok {
...
func (f *fakeStripe) Usage(itemID, period string) (float64, bool) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeStripe) next(prefix string) string {
	f.seq++
...
func (f *fakeStripe) SeedSubscription(platformCustomerID, subPrice, meteredPrice string) (customerHandle, subRef, meteredItemID string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	cus := f.next("cus")
...
func (f *fakeStripe) SeedOwnInvoiceLine(customerHandle, period, priceRef, description string) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeStripe) SeedTokenLine(customerHandle, period, kind, description string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	inv := f.invoiceFor(customerHandle)
	it := &fakeItem{
...
func (f *fakeStripe) invoiceFor(customer string) *fakeInvoice {
...
func (f *fakeStripe) route(r *http.Request, form url.Values) (int, []byte) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeStripe) createCustomer(form url.Values) (int, []byte) {
...
func (f *fakeStripe) searchCustomers(form url.Values) (int, []byte) {
	q := form.Get("query")
...
func (f *fakeStripe) createSubscription(form url.Values) (int, []byte) {
...
func (f *fakeStripe) subObject(s *fakeSub) map[string]any {
...
func (f *fakeStripe) getSubscription(id string) (int, []byte) {
	s, ok := f.subs[id]
	if !ok {
...
func (f *fakeStripe) listSubscriptions(form url.Values) (int, []byte) {
	cus := form.Get("customer")
	data := []any{}
...
func (f *fakeStripe) recordUsage(item string, form url.Values) (int, []byte) {
	if !f.hasItem(item) {
...
func (f *fakeStripe) hasItem(item string) bool {
...
func (f *fakeStripe) usageSummaries(item string) (int, []byte) {
	data := []any{}
...
func (f *fakeStripe) createInvoiceItem(form url.Values) (int, []byte) {
	cus := form.Get("customer")
	if _, ok := f.customers[cus]; !ok {
		return errBody(http.StatusNotFound, "invalid_request_error", "resource_missing", "no such customer: "+cus)
	}
...
func (f *fakeStripe) itemObject(it *fakeItem) map[string]any {
	return map[string]any{
...
func (f *fakeStripe) getInvoiceItem(id string) (int, []byte) {
	it, ok := f.items[id]
	if !ok {
...
func (f *fakeStripe) listInvoices(form url.Values) (int, []byte) {
	cus := form.Get("customer")
	data := []any{}
	for _, inv := range f.invoices {
		if inv.customer != cus {
...
func (f *fakeStripe) createCreditNote(form url.Values) (int, []byte) {
...
func (f *fakeStripe) createRefund(form url.Values) (int, []byte) {
