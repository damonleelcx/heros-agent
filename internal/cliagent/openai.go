...
func OpenAITools(opts ToolOptions) []map[string]any {
	localShell := map[string]any{
		...
		"function": map[string]any{
			"name":        "heros_shell",
			"description": "PRIMARY way to observe and execute in the user's project: runs on THIS machine with cwd = workspace root. REQUIRED before answering questions about repo layout, purpose, stack, or files (use dir/ls, type/cat README*, package.json, src/, etc.). Also use it for real artifact generation commands (for example creating .pptx/.pdf with an actual generator). Do not claim you lack context without calling this first. Prefer read-only commands first when exploring; Windows: dir, type, where; Unix: ls, cat, find.",
	...
	rest := []map[string]any{
		...
		},
		{
			...
			"function": map[string]any{
				...
				"description": "Create or update files directly on local filesystem. Use this to implement requested file changes instead of only describing steps. For binary deliverables (e.g., .pptx/.pdf/.docx), write only valid generated bytes (usually from heros_shell tooling), not plain text placeholders.",
				"parameters": map[string]any{
					"type": "object",
					"properties": map[string]any{
						"path":        map[string]any{"type": "string", "description": "Absolute path or path relative to workspace root"},
