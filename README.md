...
The URL above is pinned to the `v0.22.0` tag, so it cannot change under you, and the script is covered by the same signed manifest as the binaries. To read it before running it:

```sh
curl -fsSLO https://raw.githubusercontent.com/damonleelcx/heros-agent/v0.22.0/scripts/install.sh
